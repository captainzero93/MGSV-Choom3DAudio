...
inline void ReadConfig(std::istream& input,Config& c){
 ...
 while(std::getline(input,line)){
  ...
  value=Trim(value);
  bool* target=nullptr;
#define CHOOM_BOOL(name) if(key==#name)target=&c.name;
  CHOOM_BOOL(enableAudioBridge) CHOOM_BOOL(audioDebugLog) CHOOM_BOOL(audioBedCentreDirect) CHOOM_BOOL(audioPeakGuard)
  CHOOM_BOOL(audioFoxHooks) CHOOM_BOOL(audioNativeHooks) CHOOM_BOOL(audioListenerHook)
  CHOOM_BOOL(audioMixerHook) CHOOM_BOOL(enableNullGuard) CHOOM_BOOL(enableViewGuard)
#undef CHOOM_BOOL
  if(target){if(value=="true")*target=true;else if(value=="false")*target=false;else ++c.invalid;}
  else if(key=="audioSpatialEmphasis"){
   char* end=nullptr;float f=std::strtof(value.c_str(),&end);
   if(end==value.c_str() || *end || !std::isfinite(f) || f<0 || f>1)++c.invalid;
   else c.audioSpatialEmphasis=f;
  }
 }
}
}
