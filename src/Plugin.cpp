    ...
    DWORD WINAPI Start(LPVOID)
    {
        try
        {
            ...
            spdlog::info(
                ...
                configPath.filename() == L"choomaudio.lua",
                loaded,
                config.invalid,
                config.enableAudioBridge,
                config.audioSpatialEmphasis,
                config.audioPeakGuard,
                config.audioBedCentreDirect,
                config.enableNullGuard,
                config.enableViewGuard);
            if (HasAudioBridge(GetModuleHandleW(L"dinput8.dll")) || HasAudioBridge(GetModuleHandleW(L"IHHook.dll")))
            {
                ...
                {
                    spdlog::error(
                        "Modded IHHook found. Use your plugin-capable stock IHHook, or set enableAudioBridge=false, enableNullGuard=false, "
                        "enableViewGuard=false in the ROOT ihhook_config.lua. Plugin settings belong in plugins/choomaudio.lua. No hooks installed.");
                    return 0;
                }
            }
            ...
            if (config.enableAudioBridge)
            {
                ...
                MGSVAudioIdentityBridge::Configure(
                    ...
                    false,
                    config.audioSpatialEmphasis,
                    config.audioDebugLog,
                    config.audioBedCentreDirect,
                    config.audioPeakGuard,
                    config.audioFoxHooks); // no overlay, no hotkeys
...
BOOL WINAPI DllMain(HINSTANCE instance, DWORD reason, LPVOID)
{
    if (reason == DLL_PROCESS_ATTACH)
    {
        ...
        if (!GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_PIN, reinterpret_cast<LPCWSTR>(&DllMain), &pinned))
            return FALSE;
