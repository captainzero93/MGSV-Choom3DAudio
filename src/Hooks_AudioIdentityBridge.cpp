...
namespace MGSVAudioIdentityBridge
{
    namespace
    {
        ...
        using PlayingEventPackFn = void(__fastcall*)(std::uint32_t* outPair, std::uint32_t playingId, std::uint32_t eventId);
        ...
        using SourceToDestinationMixFn = void(__fastcall*)(std::int64_t destination, std::int64_t sourceBuffer, const float* coefficientBlock);

        using AlternateMixCoreFn = void(
            __fastcall*)(std::int64_t frameInfo, std::int64_t sourceBuffer, std::int64_t destinationBuffer, char directSameMask, const float* coefficientBlock);

        // FUN_1403D67B0(sourcePCM, destinationPCM, gainStart, gainDelta, frames)
        using LowLevelChannelMixFn =
            void(__fastcall*)(std::int64_t sourcePcm, std::int64_t destinationPcm, float gainStart, float gainDelta, std::uint32_t frames);
        ...
        CoverageSlot* FindCoverageSlot(std::uintptr_t body, std::uint32_t eventId, bool create)
        {
            ...
            for (std::size_t probe = 0; probe < 32; ++probe)
            {
                ...
                if (existing == sig)
                {
                    ...
                    if ((existingBody == 0 || existingBody == body) && (existingEvent == 0 || existingEvent == eventId))
                    {
                        if (existingBody == 0)
                ...
                if (existing == 0 && create)
                {
                    ...
                    if (slot.signature.compare_exchange_strong(expected, sig, std::memory_order_acq_rel, std::memory_order_acquire))
                    {
                        slot.body.store(body, std::memory_order_release);
                        slot.eventId.store(eventId, std::memory_order_release);
                        return &slot;
        ...
        void RememberUniqueAtomic(std::array<std::atomic<T>, N>& values, T value)
        {
            ...
            for (auto& cell : values)
            {
                ...
                {
                    T expected = 0;
                    if (cell.compare_exchange_strong(expected, value, std::memory_order_acq_rel, std::memory_order_acquire) || expected == value)
                    {
                        return;
                    }
                }
        ...
        void ObserveCaller(std::array<CallerCensusSlot, kCallerCensusSlots>& census, std::uintptr_t caller, bool tracked)
        {
            if (caller == 0)
                return;
            ...
            for (std::size_t probe = 0; probe < 16; ++probe)
            {
                ...
                if (existing == 0)
                {
                    ...
                    if (slot.caller.compare_exchange_strong(expected, caller, std::memory_order_acq_rel, std::memory_order_acquire) || expected == caller)
                    {
                        slot.calls.fetch_add(1, std::memory_order_relaxed);
                        if (tracked)
                            slot.tracked.fetch_add(1, std::memory_order_relaxed);
        ...
        void
        CoverageObserveSource(std::uintptr_t body, std::uint32_t eventId, std::uint32_t source, std::uint64_t key, std::uint32_t playingId, std::uint64_t now)
        {
        ...
        bool CheckExactExecutable()
        {
            ...
            if (dos->e_magic != IMAGE_DOS_SIGNATURE)
                return false;
            const auto* nt = reinterpret_cast<const IMAGE_NT_HEADERS64*>(g_exeBase + static_cast<std::uintptr_t>(dos->e_lfanew));
            ...
            if (nt->FileHeader.TimeDateStamp != kExpectedPeTimestamp)
            {
                if (g_log)
                {
                    g_log->error("BUILD_MISMATCH,expected_timestamp=0x{:08X},actual_timestamp=0x{:08X}", kExpectedPeTimestamp, nt->FileHeader.TimeDateStamp);
                }
                return false;
            }
            ...
            if (g_log)
            {
                g_log->info("BUILD_OK,base=0x{:016X},image_size=0x{:X},timestamp=0x{:08X}", g_exeBase, g_imageSize, nt->FileHeader.TimeDateStamp);
        ...
        struct Scope040
        {
            ...
            Scope040(std::uint32_t kind, std::uintptr_t caller, std::uintptr_t owner, std::uintptr_t source = 0, std::uintptr_t destination = 0)
                : src(source)
                , dst(destination)
            {
            ...
            for (unsigned i = 0; i < 4096; ++i)
            {
                ...
                for (unsigned side = 0; side < 2; ++side)
                {
                    ...
                    if (!b.header)
                        continue;
                    g_log->info(
                        "B41_BUFFER,seq={},side={},header=0x{:X},pcm=0x{:X},mask={},stride={},frames={},reads={},node=0x{:X},wrapper=0x{:X},context=0x{:X},"
                        "playing={},a8=0x{:X},a148=0x{:X}",
                        e.seq,
                        side,
                        b.header,
                        b.pcm,
        ...
        void Control040()
        {
            ...
            if (down && !prior)
            {
                ...
                else if (!reported)
                {
                    ...
                    if (g_log)
                        g_log->info("B41_START,session={},qpc_hz={},duration_ms=20000", g_session040.load(), hz.QuadPart);
                }
            }
            prior = down;
            ...
            if (reported && !g_capture040.load())
            {
                if (g_log)
                {
                    g_log->info("B41_STOP,session={},calls={},queue_drops_total={}", g_session040.load(), g_budget040.load(), g_drop040.load());
                    g_log->flush();
                }
                reported = false;
            }
        ...
        {
            Scope040 t(7, reinterpret_cast<std::uintptr_t>(_ReturnAddress()), bus, bus ? bus + 0x420 : 0, destinationHeader);
            t.Emit(0);
        ...
        {
            if (wma || b.audioBytes == 0 || b.audioBytes > sizeof(g_silence039) || b.audioData == nullptr || b.flags != 0 || b.playBegin != 0
                || b.playLength != 0 || b.loopBegin != 0 || b.loopLength != 0 || b.loopCount != 0)
                return false;
            for (auto& slot : g_observedSinks039)
            {
                ...
                if (!SafeReadValue(sink, vt) || vt != g_exeBase + 0x21358F0ull || !SafeReadValue(sink + 0x70, sourceVoice)
                    || sourceVoice != reinterpret_cast<std::uintptr_t>(voice) || !SafeReadValue(sink + 0x58, channels) || channels == 0 || channels > 8
                ...
                {
                    std::uintptr_t pcm = 0;
                    if (SafeReadValue(sink + 0x88 + i * 8u, pcm) && pcm == reinterpret_cast<std::uintptr_t>(b.audioData))
                        return true;
                }
            }
            return false;
        ...
        long __fastcall SubmitHook039(void* voice, const SubmitBuffer039* input, const void* wma)
        {
            ...
            const bool eligible = SafeReadValue(reinterpret_cast<std::uintptr_t>(input), local) && IsOurRingBuffer039(voice, local, wma);
            if (eligible)
            {
                ++g_submitEligible039;
        ...
        void EnsureSubmitHook039()
        {
            ...
            for (auto& slot : g_observedSinks039)
            {
                ...
                if (!SafeReadValue(sink, vt) || vt != g_exeBase + 0x21358F0ull || !SafeReadValue(sink + 0x70, voice) || !voice || !SafeReadValue(voice, voiceVt)
                    || !voiceVt || !SafeReadValue(voiceVt + 0xA8, target) || !target)
                    continue;
                MEMORY_BASIC_INFORMATION mbi{};
                if (!VirtualQuery(reinterpret_cast<void*>(target), &mbi, sizeof(mbi)) || mbi.State != MEM_COMMIT || (mbi.Protect & PAGE_GUARD))
                    continue;
                const auto protect = mbi.Protect & 0xFFu;
                if (protect != PAGE_EXECUTE && protect != PAGE_EXECUTE_READ && protect != PAGE_EXECUTE_READWRITE && protect != PAGE_EXECUTE_WRITECOPY)
                    continue;
                int expected = 0;
                ...
                if (g_log)
                    g_log->info(
                        "SUBMIT_DISCOVERY,sink=0x{:X},voice=0x{:X},voice_vtable=0x{:X},target=0x{:X},module={}",
                        sink,
                        voice,
                        voiceVt,
                        target,
                ...
                const auto created = MH_CreateHook(reinterpret_cast<void*>(target), reinterpret_cast<void*>(&SubmitHook039), &trampoline);
                ...
                if (g_log)
                {
                    g_log->info("SUBMIT_HOOK_INSTALL,state={},enable_status={}", enabled == MH_OK ? 2 : -1, static_cast<int>(enabled));
                    g_log->flush();
                }
                return;
            }
        ...
        void LogSubmitSummary039()
        {
            if (!g_log)
                return;
            g_log->info(
                "SUBMIT_SUMMARY,hook_state={},calls={},eligible={},other={},mute_attempts={},mute_accepted={},submit_failed={},mute_input_nonzero_blocks={},"
                "mute_input_peak_micro={},pcm_probe_failed={},last_bytes={},last_result=0x{:08X},last_voice=0x{:X},gate_zero_calls={},gate_nonzero_calls={},"
                "gate_unreadable_calls={}",
                g_submitHookState039.load(),
                g_submitCalls039.load(),
                g_submitEligible039.load(),
                g_submitOther039.load(),
                g_muteAttempts039.load(),
                g_muteAccepted039.load(),
                g_submitFailed039.load(),
                g_muteEnergy039.load(),
                g_mutePeakMicro039.load(),
                g_probeBadPcm039.load(),
                g_lastBytes039.load(),
                g_lastResult039.load(),
                g_lastVoice039.load(),
                g_gateZero039.load(),
                g_gateNonzero039.load(),
        ...
        std::uint32_t FindRecordsByObjectKey(std::uint64_t objectKey, std::uint32_t& source, std::uintptr_t& record)
        {
            ...
            if (base == 0 || !IsReadable(reinterpret_cast<const void*>(base), kRegisteredSourceCount * kRegisteredSourceRecordSize))
                return 0;
        ...
        RecentFoxSlot* FindFoxSlot(std::uint32_t source, bool create)
        {
            ...
            for (std::size_t probe = 0; probe < 8; ++probe)
            {
                ...
                if (existing == 0 && create)
                {
                    ...
                    if (slot.source.compare_exchange_strong(expected, source, std::memory_order_acq_rel, std::memory_order_acquire))
                    {
                        return &slot;
                    }
                    if (expected == source)
        ...
        void RememberFoxTransform(std::uint32_t source, std::uintptr_t body, const std::uint32_t (&payload)[8], std::uint64_t tick)
        {
            auto* slot = FindFoxSlot(source, true);
            if (!slot)
                return;
        ...
        bool ShouldEmitTransform(std::uint32_t source, std::uint64_t now)
        {
            ...
            for (;;)
            {
                if (now >= prior && now - prior < kTransformLogIntervalMs)
                    return false;
                if (slot->transformLogTick.compare_exchange_weak(prior, now, std::memory_order_relaxed, std::memory_order_relaxed))
                {
                    return true;
                }
            }
            ...
            for (std::size_t probe = 0; probe < 8; ++probe)
            {
                ...
                if (existing == body)
                {
                    ...
                    for (;;)
                    {
                        if (now >= prior && now - prior < kTransformLogIntervalMs)
                            return false;
                        if (slot.tick.compare_exchange_weak(prior, now, std::memory_order_relaxed, std::memory_order_relaxed))
                            return true;
                    }
                }
                if (existing == 0)
                {
                    std::uintptr_t expected = 0;
                    if (slot.body.compare_exchange_strong(expected, body, std::memory_order_acq_rel, std::memory_order_acquire) || expected == body)
                    {
        ...
        RecentKeySlot* FindKeySlot(std::uint64_t key, bool create)
        {
            ...
            for (std::size_t probe = 0; probe < 8; ++probe)
            {
                ...
                {
                    std::uint64_t expected = 0;
                    if (slot.key.compare_exchange_strong(expected, key, std::memory_order_acq_rel, std::memory_order_acquire))
                    {
                        return &slot;
                    }
                    if (expected == key)
        ...
        bool ShouldTraceFirstLookup(std::uint64_t key, std::uintptr_t object)
        {
            ...
            if (!slot)
                return false;
            std::uintptr_t expected = 0;
            if (slot->lookupTraceObject.compare_exchange_strong(expected, object, std::memory_order_acq_rel, std::memory_order_acquire))
            {
        ...
        RecentObjectSlot* FindObjectSlot(std::uintptr_t object, bool create)
        {
            ...
            for (std::size_t probe = 0; probe < 8; ++probe)
            {
                ...
                {
                    std::uintptr_t expected = 0;
                    if (slot.object.compare_exchange_strong(expected, object, std::memory_order_acq_rel, std::memory_order_acquire))
                    {
                        return &slot;
                    }
                    if (expected == object)
        ...
        RecentPlayingSlot* FindPlayingSlot(std::uint32_t playingId, bool create)
        {
            ...
            for (std::size_t probe = 0; probe < 8; ++probe)
            {
                ...
                {
                    std::uint32_t expected = 0;
                    if (slot.playingId.compare_exchange_strong(expected, playingId, std::memory_order_acq_rel, std::memory_order_acquire))
                    {
                        return &slot;
                    }
        ...
        bool SameIdentity(const IdentityMeta& a, const IdentityMeta& b)
        {
            if (a.key == 0 || b.key == 0)
                return false;
            return a.key == b.key && a.eventId == b.eventId && a.source == b.source && a.body == b.body && a.record == b.record;
        }
        ...
        bool PublishLatestListenerTransform(std::int64_t ownerValue)
        {
            ...
            if (listener == 0 || !IsReadable(reinterpret_cast<const void*>(listener + 0x40), 8 * sizeof(std::uint32_t)))
            {
                return false;
            }
        ...
        bool
        ComputeDirectGeometry(const IdentityMeta& meta, float& outAzimuthDeg, float& outElevationDeg, float& outDistance, const float* nativeWorld050 = nullptr)
        {
        ...
        BinauralHistoryState& GetBinauralHistory(std::uintptr_t node, std::uintptr_t destination)
        {
            ...
            const std::uint64_t key =
                ((static_cast<std::uint64_t>(node >> 4) * 0x9E3779B97F4A7C15ull) ^ (static_cast<std::uint64_t>(destination >> 4) * 0xC2B2AE3D27D4EB4Full))
                >> 54;
            ...
            float elevationDeg = 0.0f)
        {
            ...
            const float targetElevation =
                g_elevation059.load(std::memory_order_relaxed) && std::isfinite(elevationDeg) ? (std::min)(90.0f, (std::max)(-90.0f, elevationDeg)) : 0.0f;
            ...
            while (angleStep < -180.0f)
                angleStep += 360.0f;
            const bool crossfade056 = std::fabs(angleStep) > 0.01f || std::fabs(smoothedElevation - state.priorElevationDeg) > 0.01f;
            if (crossfade056)
            {
            ...
            for (std::uint32_t i = 0; i < frames; ++i)
            {
                ...
                blockPeak = (std::max)(blockPeak, (std::max)(std::fabs(g_directLeftEar[i]), std::fabs(g_directRightEar[i])));
            ...
            const float* nativeWorld050 = nullptr)
        {
            ...
            if (!SafeReadValue(sourceBuffer + 0x00, originalData) || !SafeReadValue(sourceBuffer + 0x10, maxFrames16)
            ...
            if (sourceMask == 0x00000004u)
            {
                if (!IsReadable(coefficientBlock, 16u * sizeof(float))
                    || !IsReadable(reinterpret_cast<const void*>(originalData), static_cast<std::size_t>(frames) * sizeof(float)))
                {
                    g_directHrtfBadPcm.fetch_add(1, std::memory_order_relaxed);
                    return false;
                }
            ...
            else
            {
                ...
                if (!PrepareWideWorldMono(sourceBuffer, sourceMask, coefficientBlock, frames, monoCoefficients, sourcePeak))
                {
            ...
            if (!bypass044 && sourceMask == 0x00000003u && g_stereoSpread056.load(std::memory_order_relaxed) && std::isfinite(distance) && distance > 0.0f)
            {
            ...
            else if (!PrepareDirectMeasuredHrir(hrtfSource, frames, node, destination, azimuthDeg, pure046, elevationDeg))
            {
                g_directHrtfBadPcm.fetch_add(1, std::memory_order_relaxed);
                return false;
            }
            ...
            *reinterpret_cast<std::uintptr_t*>(fakeBuffer.data() + 0x00) = reinterpret_cast<std::uintptr_t>(g_directLeftEar);
            ...
            *reinterpret_cast<std::uintptr_t*>(fakeBuffer.data() + 0x00) = reinterpret_cast<std::uintptr_t>(g_directRightEar);
        ...
        void RenewVerifiedIdentity(std::uint32_t playingId, std::uintptr_t object, const IdentityMeta& meta, std::uint64_t now)
        {
            if (playingId != 0)
            {
        ...
        bool ShouldEmitMatrixBind(std::uintptr_t context, std::uint64_t now)
        {
            ...
            for (std::size_t probe = 0; probe < 8; ++probe)
            {
                ...
                {
                    std::uintptr_t expected = 0;
                    if (slot.context.compare_exchange_strong(expected, context, std::memory_order_acq_rel, std::memory_order_acquire) || expected == context)
                    {
                        slot.tick.store(now, std::memory_order_relaxed);
                        return true;
                    }
            ...
            const IdentityMeta& meta)
        {
            ...
            if (g_log && CanLog())
            {
                g_log->info(
                    ...
                    playingId,
                    meta.key,
                    static_cast<std::uint64_t>(meta.body),
                    meta.source,
        ...
        void ScanRenderRegion(const RenderSnapshot& snap, std::uint32_t region, const std::uint8_t* bytes, std::size_t size, std::size_t& matches)
        {
            ...
            for (std::size_t off = 0; off + 4 <= size && matches < 16; off += 4)
            {
                ...
                {
                    IdentityMeta meta{};
                    if (LookupPlaying(v32, meta, now) && LogRenderIdentityMatch(snap, region, static_cast<std::uint32_t>(off), "playing32", v32, v32, meta))
                    {
                        ++matches;
                    }
                ...
                if (off + 8 <= size && matches < 16)
                {
                    ...
                    if (v64 != 0)
                    {
                        ...
                        if (hi == snap.eventId && LookupPlaying(lo, meta, now)
                            && LogRenderIdentityMatch(snap, region, static_cast<std::uint32_t>(off), "packed_playing_event", v64, lo, meta))
                        {
                            ++matches;
                        }
                        meta = IdentityMeta{};
                        if (lo == snap.eventId && LookupPlaying(hi, meta, now)
                            && LogRenderIdentityMatch(snap, region, static_cast<std::uint32_t>(off), "packed_event_playing", v64, hi, meta))
                        {
                            ++matches;
                        }
                    }
        ...
        void FlushCensus049(bool final = false)
        {
            ...
            for (std::size_t i = 0; i < limit; ++i)
            {
                ...
                if (emit)
                {
                    ++g_censusRows049;
                    g_log->info(
                        "SOURCE049,kind={},a=0x{:X},b=0x{:X},c=0x{:X},first={},last={},calls={},applied={},event=0x{:X},playing={},key=0x{:X},body=0x{:X},"
                        "source=0x{:X},node=0x{:X},object=0x{:X},caller=0x{:X},routes={},match_bits={},src_masks_or={},dst_masks_or={},playing_stage_bits={},"
                        "object_stage_bits={}",
                        out.kind,
                        out.key1,
                        out.key2,
                    ...
                    if (out.kind == 4)
                        g_log->info(
                            "POSITION050,a=0x{:X},b=0x{:X},c=0x{:X},key=0x{:X},x_bits={},y_bits={},z_bits={}",
                            out.key1,
                            out.key2,
                            out.key3,
                            out.objectKey,
            ...
            {
                g_log->info(
                    "CENSUS049,tick={},unique={},rows={},busy_drops={},table_drops={},row_cap={},final={}",
                    now,
                    g_censusUnique049.load(),
                    g_censusRows049,
                    g_censusBusy049.load(),
                    g_censusFull049.load(),
                    g_censusRows049 >= 200000 ? 1 : 0,
                    final ? 1 : 0);
                g_log->flush();
                summary = now;
            }
        ...
        void EnqueueTrace(const TracePayload& p)
        {
            ...
            c.event = p.eventId;
            c.playing = p.kind == TraceKind::WwisePost ? static_cast<std::uint32_t>(p.result) : static_cast<std::uint32_t>(p.nestedPostResult);
            c.key1 = p.eventId;
            c.key2 = p.objectKey;
            c.key3 = c.playing;
            if (c.kind == 2)
        ...
        void FlushTraceRecords()
        {
            ...
            bool expected = false;
            if (!g_flushBusy.compare_exchange_strong(expected, true, std::memory_order_acq_rel, std::memory_order_relaxed))
            {
                return;
            }
                ...
                if (g_log && CanLog())
                {
                    ...
                    case TraceKind::SourceRecord:
                        g_log->info(
                            ...
                            p.candidateStoredSource,
                            p.candidateStoredKey64,
                            p.derivedObjectKey,
                            p.derivedKeyMatch,
                            p.nestedObjectKey,
                    ...
                    case TraceKind::PlayAction:
                        g_log->info(
                            ...
                            p.tick,
                            p.tid,
                            static_cast<std::uint64_t>(p.action),
                            p.playingId,
                            p.matchMode,
                            static_cast<std::uint64_t>(p.object),
                            p.objectKey,
                    ...
                    case TraceKind::LowLevelRoute:
                        g_log->info(
                            ...
                            p.destinationMask,
                            p.lowFrames,
                            FloatFromBits(p.gainStartBits),
                            FloatFromBits(p.gainDeltaBits),
                            static_cast<std::uint64_t>(p.matrixCaller),
            ...
            if (g_log && now - prior >= kSummaryIntervalMs
                && g_summaryTick.compare_exchange_strong(prior, now, std::memory_order_relaxed, std::memory_order_relaxed))
            {
                g_log->info(
        ...
        int* __fastcall SourcePostEventHook(std::int64_t body, int* outResult, std::uint32_t eventId)
        {
            ...
            InspectRecordCandidate(sourceForRecord, tableBase, candidateRecord, candidateStoredSource, candidateStoredKey);

            if (sourceForRecord != 0 && candidateStoredSource == sourceForRecord)
            {
                g_tableSourceMatches.fetch_add(1, std::memory_order_relaxed);
            }
            if (hadPending && pendingCopy.sawMatchingWwisePost && candidateStoredKey == pendingCopy.matchingObjectKey && candidateStoredKey != 0)
            {
                g_tableKeyMatches.fetch_add(1, std::memory_order_relaxed);
            }

            const std::uint64_t derivedKey = sourceForRecord != 0 ? static_cast<std::uint64_t>((sourceForRecord >> 8) + 1u) : 0ull;
            const bool derivedKeyMatch = hadPending && pendingCopy.sawMatchingWwisePost && derivedKey != 0 && derivedKey == pendingCopy.matchingObjectKey;
            if (derivedKeyMatch)
            {
            ...
            const std::uint32_t coveragePlaying =
                hadPending && pendingCopy.matchingPostResult > 0 ? static_cast<std::uint32_t>(pendingCopy.matchingPostResult) : 0u;
            const std::uint64_t coverageKey = hadPending && pendingCopy.sawMatchingWwisePost ? pendingCopy.matchingObjectKey : objectKey;
            ...
            std::uint32_t param8)
        {
            ...
            const int result = g_wwisePostOriginal(eventId, objectKey, param3, param4, param5, param6, param7, param8);
        ...
        void __fastcall VoiceRenderNodeHook(std::int64_t audioBufferLike)
        {
            ...
            const bool playingMatch = playingId != 0 && LookupPlaying(playingId, playingMeta, now) && playingMeta.body != 0;
            const bool objectA8Match = objectA8 != 0 && LookupObject(objectA8, objectMetaA8, now) && objectMetaA8.body != 0;
            const bool object148Match = object148 != 0 && LookupObject(object148, objectMeta148, now) && objectMeta148.body != 0;
            ...
            if (selected.body != 0)
            {
                ...
                const auto selectedPlaying = playingId != 0 ? playingId : (selected.postResult > 0 ? static_cast<std::uint32_t>(selected.postResult) : 0u);
                CoverageObserveRender(selected.body, selected.eventId, selected.source, selected.key, selectedPlaying, matchMask, now);

                // Refresh identity while the live context still references the same object or playing ID.
                const std::uintptr_t verifiedObject = (playingMatch && objectA8Match && SameIdentity(selected, objectMetaA8))
                                                          ? objectA8
            ...
            if (selected.body != 0)
            {
                ...
                g_renderTls.playingId = playingId != 0 ? playingId : (selected.postResult > 0 ? static_cast<std::uint32_t>(selected.postResult) : 0u);
        ...
        bool WorldEligible047(
            bool dual,
            std::uintptr_t body,
            std::uint32_t playing,
            unsigned route,
            unsigned sourceMask,
            unsigned destinationMask,
            std::uint16_t frames)
        {
            return dual && body && playing && route == 2 && destinationMask == 3 && frames && (sourceMask == 4 || sourceMask == 3 || sourceMask == 0x33);
        }

        void PrepareWorldHistory047(BinauralHistoryState& history, std::uintptr_t body, std::uint32_t playing, std::uint64_t epoch, std::uint64_t now)
        {
            if (history.ownerBody047 != body || history.ownerPlaying047 != playing || history.ownerEpoch047 != epoch || now - history.lastTick047 > 250)
            {
        ...
        void Control047()
        {
            ...
            if (g_log && (mode != old || now - last >= 60000))
            {
                g_log->info(
                    ...
                    mode,
                    g_worldAttempts047.load(),
                    g_worldApplied047.load(),
                    g_worldFallback047.load(),
                    g_worldMono047.load(),
                    g_worldStereo047.load(),
                    g_worldQuad047.load(),
                    g_worldLayout047.load(),
                    g_binauralHistoryResets.load());
                g_log->info(
                    "COVERAGE048,tick={},route2={},dual={},playing_missing_key={},playing_no_slot={},playing_expired={},playing_no_body={},playing_bound={},"
                    "object_missing_key={},object_no_slot={},object_expired={},object_no_body={},object_bound={},activity={}",
                    now,
                    g_route2Seen048.load(),
                    g_route2Dual048.load(),
                    g_playingStages048[0].load(),
                    g_playingStages048[1].load(),
                    g_playingStages048[2].load(),
                    g_playingStages048[3].load(),
                    g_playingStages048[4].load(),
                    g_objectStages048[0].load(),
                    g_objectStages048[1].load(),
                    g_objectStages048[2].load(),
                    g_objectStages048[3].load(),
                    g_objectStages048[4].load(),
        ...
        std::atomic<std::uint64_t> g_nativeCandidates050{ 0 }, g_nativeApplied050{ 0 }, g_nativeMuted050{ 0 }, g_nativeFallback050{ 0 },
            g_nativeAmbiguous050{ 0 };
        ...
        std::atomic<std::uint64_t> g_setOutOfScope052{ 0 }, g_setKeyMismatch052{ 0 }, g_setMode052{ 0 }, g_setStorage052{ 0 }, g_setNonFinite052{ 0 },
            g_setCountZero052{ 0 };
...
#define NATIVE_CALLER052() reinterpret_cast<std::uintptr_t>(__builtin_return_address(0))
        ...
        void __fastcall NativeSetHook050(std::uintptr_t object, const float* positions, std::uint16_t count, std::uint32_t mode)
        {
            ...
            float stored[3]{};
            bool valid = captured && mode <= 2 && g_nativeScope050 && SafeReadValue(object + 0x70, key) && key && key == g_nativeScopeKey050
                         && SafeReadValue(object + 0x10, storedCount) && storedCount == 1 && SafeReadValue(object + 8, data) && data
                         && SafeReadValue(data, stored);
        ...
        NativeDispatchHook050(std::uintptr_t manager, std::uint64_t key, const float* positions, std::uint16_t count, std::uint32_t mode)
        {
        ...
        bool ReadDirect053(std::uintptr_t object, NativePosition050& out)
        {
            ...
            std::uintptr_t data = 0;
            float xyz[3]{};
            if (!SafeReadValue(object + 0x70, key) || !key || !SafeReadValue(object + 0x10, count) || count != 1 || !SafeReadValue(object + 0x13, flags13)
                || (flags13 & 7u) > 2u || !SafeReadValue(object + 8, data) || !data || !SafeReadValue(data, xyz))
                return false;
            for (unsigned i = 0; i < 3; ++i)
        ...
        void ControlNative050()
        {
            ...
            {
                g_log->info(
                    "NATIVE050,tick={},ready={},enabled={},mute={},updates={},valid_updates={},multi_updates={},drops={},candidates={},applied={},muted={},"
                    "fallback={},ambiguous={}",
                    now,
                    g_nativeReady050.load() ? 1 : 0,
                    enabled ? 1 : 0,
                    mute ? 1 : 0,
                    g_nativeUpdates050.load(),
                    g_nativeValid050.load(),
                    g_nativeMulti050.load(),
                    g_nativeDrops050.load(),
                    g_nativeCandidates050.load(),
                    g_nativeApplied050.load(),
                    g_nativeMuted050.load(),
                    g_nativeFallback050.load(),
                    g_nativeAmbiguous050.load());
                g_log->info(
                    "NATIVE051,tick={},state={},cand_mono={},cand_stereo={},cand_quad={},applied_mono={},applied_stereo={},applied_quad={}",
                    now,
                    !enabled ? "off" : (g_nativeWide051.load() ? "mono_stereo_quad" : "mono"),
                    g_nativeCandMono051.load(),
                    g_nativeCandStereo051.load(),
                    g_nativeCandQuad051.load(),
                    g_nativeAppliedMono051.load(),
                    g_nativeAppliedStereo051.load(),
                    g_nativeAppliedQuad051.load());
                ...
                g_log->info(
                    ...
                    now,
                    g_setOutOfScope052.load(),
                    g_setKeyMismatch052.load(),
                    g_setMode052.load(),
                    g_setStorage052.load(),
                    g_setNonFinite052.load(),
                    g_setCountZero052.load(),
                    a5(g_singleFail052),
                    a5(g_pairA8Fail052),
                    a5(g_pair148Fail052),
                    g_pairBothFail052.load(),
                    g_pairKeyDiff052.load(),
                    g_pairPosDiff052[0].load(),
                    g_pairPosDiff052[1].load(),
                    g_pairPosDiff052[2].load(),
                    g_pairPosDiff052[3].load(),
                    g_gateNoPlaying052.load(),
                    g_gateDest052.load(),
                    g_gateLayout052.load(),
                    g_gateLfe052.load(),
                    g_foxMuted052.load());
                g_log->info(
                    "NATIVE053,tick={},direct_reads={},a8_preferred_over_unpositioned_148={}", now, g_nativeDirect053.load(), g_pairA8Preferred053.load());
                {
                    ...
                    g_log->info(
                        ...
                        now,
                        !g_bedEnabled055.load() ? "off" : (g_bedSkip71_057.load() ? "ambience_only" : "all"),
                        g_bedLevelMatch057.load() ? 1 : 0,
                        g_bedMakeupLarge057.load(),
                        g_bedImplicit058.load());
                    g_log->info(
                        "BED055,tick={},enabled={},applied={},quad={},five={},seven={},other={},muted={},fail={},order_mismatch={},peak_guard={}",
                        now,
                        g_bedEnabled055.load() ? 1 : 0,
                        g_bedApplied055.load(),
                        g_bedQuad055.load(),
                        g_bedFive055.load(),
                        g_bedSeven055.load(),
                        g_bedOther055.load(),
                        g_bedMuted055.load(),
                        g_bedFail055.load(),
                        g_bedOrderMismatch056.load(),
                        g_bedPeakGuard055.load());
                    g_log->info(
                        "ELEV059,tick={},enabled={},blocks_below_-20={},level={},above_+20={}",
                        now,
                        g_elevation059.load() ? 1 : 0,
                        g_elevBelow059.load(),
                        g_elevLevel059.load(),
                        g_elevAbove059.load());
                    g_log->info(
                        ...
                        now,
                        g_worldMode047.load() == 2 ? "pure" : (g_worldMode047.load() == 1 ? "hybrid" : "native"),
                        g_filterCrossfades056.load(),
                        g_limiterEngaged056.load(),
                        g_stereoWide056.load(),
                        g_binauralHistoryResets.load());
        ...
        {
            return c.meta.body == m.body && c.meta.eventId == m.eventId && c.meta.key == m.key && c.meta.source == m.source && c.playing == playing;
        }
        ...
        unsigned Audition042(
            ...
            std::uint64_t now)
        {
            if (!dual || !meta.body || !playing || !frames || (route != 1 && route != 2) || !KnownLayout042(sourceMask) || !KnownLayout042(destinationMask))
                return false;
            ...
            if (i < g_candidates042.size())
            {
                ...
                if (static_cast<int>(i) == g_selected042)
                {
                    if (g_muteHeld042.load(std::memory_order_relaxed))
                        ...
                        g_bypassHeld044.load(std::memory_order_relaxed) || g_pureHeld046.load(std::memory_order_relaxed)
        ...
        void Control042()
        {
            ...
            {
                g_log->info(
                    "AUDITION046,changed={},selected={},live={},f7={},armed={},body=0x{:X},event=0x{:X},playing={},source=0x{:X},key=0x{:X},hits={},muted={},"
                    "route1_muted={},route2_muted={},age_ms={},lock_busy={}",
                    changed ? 1 : 0,
                    index,
                    live,
                    held ? 1 : 0,
                    g_muteHeld042.load() ? 1 : 0,
                    snapshot.meta.body,
                    snapshot.meta.eventId,
                    snapshot.playing,
                    ...
                    snapshot.route1,
                    snapshot.route2,
                    index >= 0 ? now - snapshot.last : 0,
                    g_candidateBusy042.load());
                g_log->info(
                    "SELECTION046,eligible={},mono_hits={},mono_age_ms={},available_mono={}",
                    ready045 ? 1 : 0,
                    snapshot.monoHits,
                    index >= 0 && now >= snapshot.monoLast ? now - snapshot.monoLast : 0,
                    live);
                g_log->info(
                    "HRTF046,f8={},armed={},attempts={},applied={},fallback={},layout_skipped={},azimuth_deg={},distance={}",
                    hrtf ? 1 : 0,
                    hrtfArmed ? 1 : 0,
                    g_hrtfAttempts043.load(),
                    g_hrtfApplied043.load(),
                    g_hrtfFallback043.load(),
                    g_hrtfLayoutSkipped043.load(),
                    FloatFromBits(g_azimuth043.load()),
                    FloatFromBits(g_distance043.load()));
                g_log->info(
                    "BYPASS046,f10={},armed={},attempts={},applied={},fallback={},metrics_mode={},input_peak={},left_peak={},right_peak={},input_rms={},left_"
                    "rms={},right_rms={}",
                    bypass ? 1 : 0,
                    g_bypassHeld044.load() ? 1 : 0,
                    g_bypassAttempts044.load(),
                    g_bypassApplied044.load(),
                    g_bypassFallback044.load(),
                    g_metricsMode044.load(),
                    FloatFromBits(g_inputPeak044.load()),
                    FloatFromBits(g_leftPeak044.load()),
                    FloatFromBits(g_rightPeak044.load()),
                    FloatFromBits(g_inputRms044.load()),
                    FloatFromBits(g_leftRms044.load()),
                    FloatFromBits(g_rightRms044.load()));
                g_log->info(
                    "PURE046,f11={},armed={},attempts={},applied={},fallback={}",
                    pure ? 1 : 0,
                    pureArmed ? 1 : 0,
                    g_pureAttempts046.load(),
                    g_pureApplied046.load(),
        ...
        bool ApplyBedVirtualizer055(
            ...
            std::uint32_t sourceMask,
            std::uint64_t now,
            std::uint32_t eventId057 = 0,
            std::uint32_t playingId057 = 0)
        {
            const auto sourceBuffer = static_cast<std::uintptr_t>(sourceBufferValue);
            std::uintptr_t data = 0;
            std::uint16_t maxFrames16 = 0, validFrames16 = 0;
            if (!sourceBuffer || !coefficientBlock || !SafeReadValue(sourceBuffer + 0x00, data) || !SafeReadValue(sourceBuffer + 0x10, maxFrames16)
            ...
            if (!IsReadable(coefficientBlock, channels * 16u * sizeof(float))
                || !IsReadable(reinterpret_cast<const void*>(data), static_cast<std::size_t>(channels) * maxFrames * sizeof(float)))
                return false;
            ...
            for (unsigned ch = 0; ch < channels; ++ch)
            {
                ...
                if ((leftBit && r > l * 1.05f + 1.0e-4f) || (rightBit && l > r * 1.05f + 1.0e-4f) || (bit == 2 && std::fabs(l - r) > 0.25f * (std::max)(l, r)))
                {
            ...
            for (unsigned ch = 0; ch < channels; ++ch)
            {
                ...
                if (!zeroRow || bit == 0 || bit == 1 || bit == 3 || !g_bedImplicitFold058.load(std::memory_order_relaxed))
                    continue;
...

            {
                ...
                if (g_log && g_debugLog063.load(std::memory_order_relaxed) && now - prev >= 2000 && last.compare_exchange_strong(prev, now))
                {
                    ...
                    g_log->info(
                        "BEDDIAG057,tick={},node=0x{:X},mask=0x{:X},event=0x{:08X},playing={},frames={},orig_db={:.1f},binaural_db={:.1f},after_makeup_db={:."
                        "1f},makeup={:.2f},ch={}",
                        now,
                        node,
                        sourceMask,
        ...
        void __fastcall SourceToDestinationMixHook(std::int64_t destinationValue, std::int64_t sourceBufferValue, const float* coefficientBlock)
        {
            const auto caller = reinterpret_cast<std::uintptr_t>(_ReturnAddress());
            ...
            if (tracked)
            {
                ...
                CoverageObserveMatrix(
                    selected.body, selected.eventId, selected.source, selected.key, playingId, route, sourceMask, destinationMask, caller, now);
                ...
                if ((hrtfRequested || muteRequested) && ShouldEmitMatrixBind(context, now))
                {
                    ...
                    trace.contextObjectA8 = objectA8;
                    trace.contextObject148 = object148;
                    trace.objectA8Match = (playingMatch && objectA8Match && SameIdentity(playingMeta, objectMetaA8)) ? 1u : 0u;
                    trace.object148Match = (playingMatch && object148Match && SameIdentity(playingMeta, objectMeta148)) ? 1u : 0u;
            ...
            if (mode047 && g_nativeMute050.load() && WorldEligible047(independent047, selected.body, playingId, route, sourceMask, destinationMask, frames047))
            {
            ...
            else if (mode047 && WorldEligible047(independent047, selected.body, playingId, route, sourceMask, destinationMask, frames047))
            {
                ++g_worldAttempts047;
                ...
                if (frames047 == stride047 && stride047 <= kBinauralMaxFrames)
                {
                    ...
                    processed047 = ApplyWideWorldHrtf(
                        destinationValue,
                        sourceBufferValue,
                        coefficientBlock,
                        node,
                ...
                mode047 && independent047 && route == 2 && frames047 && (destinationMask != 3 || (sourceMask != 4 && sourceMask != 3 && sourceMask != 0x33)))
                ++g_worldLayout047;
            ...
            const bool nativeCandidate050 = !independent047 && route == 2 && playingId && nativeLayout051 && destinationMask == 3 && frames047
            ...
            if (!processed047 && mode047 && g_bedEnabled055.load() && route == 2 && frames047 && destinationMask == 3 && BedLayout055(sourceMask)
                && !(g_bedSkip71_057.load(std::memory_order_relaxed) && __popcnt(sourceMask) == 8))
            {
                ...
                {
                    processed047 = ApplyBedVirtualizer055(
                        destinationValue, sourceBufferValue, coefficientBlock, node, destination, sourceMask, now, selected.eventId, playingId);
                    if (!processed047)
                        ++g_bedFail055;
                }
            }
            ...
            const float* coefficientBlock)
        {
            ...
            std::uintptr_t objectA8 = 0;
            std::uintptr_t object148 = 0;
            if (sourceBuffer != 0 && SafeReadValue(sourceBuffer + 0x40, node) && node != 0 && SafeReadValue(node + 0x10, wrapper) && wrapper != 0
            ...
            Scope040 trace040(
                3, reinterpret_cast<std::uintptr_t>(_ReturnAddress()), static_cast<std::uintptr_t>(frameInfoValue), sourceBuffer, destinationBuffer);
            trace040.Emit(0);
            g_alternateMixCoreOriginal(frameInfoValue, sourceBufferValue, destinationBufferValue, directSameMask, coefficientBlock);
            trace040.Emit(1);
        }

        void __fastcall LowLevelChannelMixHook(std::int64_t sourcePcm, std::int64_t destinationPcm, float gainStart, float gainDelta, std::uint32_t frames)
        {
            const auto caller = reinterpret_cast<std::uintptr_t>(_ReturnAddress());
            ...
            if (tracked)
            {
                ...
                CoverageObserveLowLevel(selected.body, selected.eventId, selected.source, selected.key, playingId, caller, viaMatrix, now);
    ...
    bool GetAudioStatus(std::string& text)
    {
        ...
        std::snprintf(
            ...
            static_cast<unsigned long long>(g_bedApplied055.load()),
            g_elevation059.load() ? "ON" : "OFF");
        text = buf;
        return true;
    }
    ...
    bool Install()
    {
        ...
        try
        {
            ...
            g_log->info(
                "CONFIG069,audioOverlay={},audioHotkeys={},audioSpatialEmphasis={:.2f},audioDebugLog={},audioBedCentreDirect={},audioPeakGuard={},"
                "audioFoxHooks={},native={},listener={},mixer={},centre_delay_taps={}",
                g_overlay062.load() ? 1 : 0,
                g_hotkeys062.load() ? 1 : 0,
                g_emphasis062.load(),
                g_debugLog063.load() ? 1 : 0,
                g_bedCentreDirect064.load() ? 1 : 0,
                g_peakGuard066.load() ? 1 : 0,
                g_foxHooks068.load() ? 1 : 0,
                g_nativeHooks069.load() ? 1 : 0,
                g_listenerHook069.load() ? 1 : 0,
                g_mixerHook069.load() ? 1 : 0,
                g_bedCentreDelay064);
            {
                double hrirSum = 0.0;
            ...
            g_log->info(
                "MODE,default=pure_world_hrtf,f8=toggle_world_hrtf,f9=native_audio,f11=switch_hybrid_pure,f6=toggle_status_overlay,f7=hold_mute_all_"
                "spatialised_and_marker,f10=cycle_native_stereo_default_on,f5=cycle_beds_all_ambience_off,f4=toggle_elevation_default_on");
            g_log->info(
                "MODE,submit_target=runtime_sink_voice_vtable_plus_0xA8,selection=validated_sink_ring_buffers,global_mix_diagnostic=0,per_emitter_hrtf="
                "verified_world_sources");
        }
        catch (...)
        {
        ...
        const bool nativeCtor =
            ...
            && InstallOne(
                0x388590, reinterpret_cast<void*>(&NativeCtorHook050), reinterpret_cast<void**>(&g_nativeCtorOriginal050), "NativeObject.Constructor050");
        const bool nativeSet =
            g_nativeHooks069.load()
            && InstallOne(
                0x388e50, reinterpret_cast<void*>(&NativeSetHook050), reinterpret_cast<void**>(&g_nativeSetOriginal050), "NativeObject.SetConsumedPosition050");
        const bool nativeDispatch = g_nativeHooks069.load()
                                    && InstallOne(
                                        0x34d220,
                                        reinterpret_cast<void*>(&NativeDispatchHook050),
        ...
        if (g_log)
            g_log->info(
                "NATIVE_INSTALL050,ctor={},setter={},dispatch={},ready={}",
                nativeCtor ? 1 : 0,
                nativeSet ? 1 : 0,
                nativeDispatch ? 1 : 0,
                g_nativeReady050.load() ? 1 : 0);
        const bool sourceEvent = g_foxHooks068.load()
                                 && InstallOne(
                                     kRvaSourcePostEvent,
        ...
        const bool sourceTransform = g_foxHooks068.load()
                                     && InstallOne(
        ...
        const bool listener = g_listenerHook069.load()
                              && InstallOne(
                                  kRvaListenerBridge,
        ...
        const bool wwisePost = g_foxHooks068.load()
                               && InstallOne(
        ...
        const bool gameObjectLookup = g_foxHooks068.load()
                                      && InstallOne(
        ...
        const bool playingEventPack = g_foxHooks068.load()
                                      && InstallOne(
        ...
        const bool voiceRender = g_foxHooks068.load()
                                 && InstallOne(
                                     kRvaVoiceRenderNode,
        ...
        const bool matrixMix = g_mixerHook069.load()
                               && InstallOne(
        ...
        if (g_log)
        {
            g_log->info(
                ...
                "wwise_post={},game_object_lookup={},play_action={},playing_event_pack={},active_render={},sink_pump={},matrix_mix={},alternate_mix={},low_"
                "level_mix={}",
                sourceEvent ? 1 : 0,
                sourceTransform ? 1 : 0,
                listener ? 1 : 0,
            ...
            g_log->info("V042_HARDENING,low_level_hook=0,play_action_hook=0,submit_descriptor_unchanged=1,live_ring_write=0,no_queue_skipping=1");
        ...
        return sourceEvent || sourceTransform || listener || wwisePost || gameObjectLookup || actionDispatch || playingEventPack || voiceRender || sinkPump
               || matrixMix || alternateMix || lowLevelMix;
    }
