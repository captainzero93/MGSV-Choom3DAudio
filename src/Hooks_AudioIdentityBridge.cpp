...
using WwisePostGatewayFn = int(__fastcall*)(std::uint32_t eventId, std::uint64_t objectKey,
                                            std::uint32_t param3, std::uint64_t param4, std::uint64_t param5,
                                            int param6, std::uint64_t param7, std::uint32_t param8);

// Call sites in FUN_140348ED0 pass DAT_142B9EAE8 and the queued object key.
using WwiseGameObjectLookupFn = std::int64_t(__fastcall*)(std::int64_t manager, std::uint64_t objectKey);

// Ghidra: void FUN_140347e00(longlong runtime, longlong *action)
using PlaybackActionDispatchFn = void(__fastcall*)(std::int64_t runtime, std::int64_t action);

// FUN_140342060 takes (outPair, PostGateway result, eventId).
// Callers queue the first 8 output bytes.
using PlayingEventPackFn = void(__fastcall*)(std::uint32_t* outPair, std::uint32_t playingId,
...
using SourceToDestinationMixFn = void(__fastcall*)(std::int64_t destination, std::int64_t sourceBuffer,
                                                   const float* coefficientBlock);

using AlternateMixCoreFn = void(__fastcall*)(std::int64_t frameInfo, std::int64_t sourceBuffer,
                                             std::int64_t destinationBuffer, char directSameMask,
                                             const float* coefficientBlock);

// FUN_1403D67B0(sourcePCM, destinationPCM, gainStart, gainDelta, frames)
using LowLevelChannelMixFn = void(__fastcall*)(std::int64_t sourcePcm, std::int64_t destinationPcm,
                                               float gainStart, float gainDelta, std::uint32_t frames);
...
// counters
std::atomic<std::uint64_t> g_filterCrossfades056{0}, g_limiterEngaged056{0}, g_stereoWide056{0},
    g_bedOrderMismatch056{0};
...
std::size_t Hash32(std::uint32_t value, std::size_t mask) {
    return static_cast<std::size_t>((static_cast<std::uint64_t>(value) * 2654435761ull) & mask);
}
...
CoverageSlot* FindCoverageSlot(std::uintptr_t body, std::uint32_t eventId, bool create) {
    if (body == 0)
        return nullptr;
    ...
    for (std::size_t probe = 0; probe < 32; ++probe) {
        ...
        if (existing == 0 && create) {
            ...
            if (slot.signature.compare_exchange_strong(expected, sig, std::memory_order_acq_rel,
...
void ObserveCaller(std::array<CallerCensusSlot, kCallerCensusSlots>& census, std::uintptr_t caller,
                   bool tracked) {
    if (caller == 0)
        return;
    ...
    for (std::size_t probe = 0; probe < 16; ++probe) {
        ...
        if (existing == caller) {
            ...
            if (tracked)
                slot.tracked.fetch_add(1, std::memory_order_relaxed);
            return;
        }
        if (existing == 0) {
            std::uintptr_t expected = 0;
            if (slot.caller.compare_exchange_strong(expected, caller, std::memory_order_acq_rel,
                                                    std::memory_order_acquire) ||
                expected == caller) {
                slot.calls.fetch_add(1, std::memory_order_relaxed);
                if (tracked)
...
void CoverageObserveSource(std::uintptr_t body, std::uint32_t eventId, std::uint32_t source,
                           std::uint64_t key, std::uint32_t playingId, std::uint64_t now) {
    ...
    if (playingId != 0)
        slot->latestPlaying.store(playingId, std::memory_order_relaxed);
    slot->sourcePosts.fetch_add(1, std::memory_order_relaxed);
}

void CoverageObserveRender(std::uintptr_t body, std::uint32_t eventId, std::uint32_t source,
                           std::uint64_t key, std::uint32_t playingId, std::uint32_t matchMask,
                           std::uint64_t now) {
    auto* slot = FindCoverageSlot(body, eventId, true);
    if (!slot)
        return;
...
void CoverageObserveMatrix(std::uintptr_t body, std::uint32_t eventId, std::uint32_t source,
                           std::uint64_t key, std::uint32_t playingId, std::uint32_t route,
                           std::uint32_t sourceMask, std::uint32_t destinationMask, std::uintptr_t caller,
                           std::uint64_t now) {
    auto* slot = FindCoverageSlot(body, eventId, true);
    if (!slot)
        return;
...
void CoverageObserveLowLevel(std::uintptr_t body, std::uint32_t eventId, std::uint32_t source,
                             std::uint64_t key, std::uint32_t playingId, std::uintptr_t caller,
                             bool viaMatrix, std::uint64_t now) {
    auto* slot = FindCoverageSlot(body, eventId, true);
    if (!slot)
        return;
    if (slot->firstTick.load(std::memory_order_relaxed) == 0)
    ...
    if (viaMatrix)
        slot->lowViaMatrix.fetch_add(1, std::memory_order_relaxed);
    else
        slot->lowViaRender.fetch_add(1, std::memory_order_relaxed);
...
bool ShouldTraceCoverageTick(std::atomic<std::uint64_t>& cell, std::uint64_t now) {
    ...
    for (;;) {
        if (prior != 0 && now >= prior && now - prior < kCoverageTraceIntervalMs)
            return false;
        if (cell.compare_exchange_weak(prior, now, std::memory_order_relaxed, std::memory_order_relaxed))
            return true;
    }
}
...
template <typename T> bool SafeReadValue(std::uintptr_t address, T& out) {
    if (address == 0 || !IsReadable(reinterpret_cast<const void*>(address), sizeof(T))) {
        return false;
    }
    T temp{};
...
bool CheckExactExecutable() {
    ...
    if (dos->e_magic != IMAGE_DOS_SIGNATURE)
        return false;
    const auto* nt =
        reinterpret_cast<const IMAGE_NT_HEADERS64*>(g_exeBase + static_cast<std::uintptr_t>(dos->e_lfanew));
    ...
    if (nt->FileHeader.TimeDateStamp != kExpectedPeTimestamp) {
        if (g_log) {
            g_log->error("BUILD_MISMATCH,expected_timestamp=0x{:08X},actual_timestamp=0x{:08X}",
                         kExpectedPeTimestamp, nt->FileHeader.TimeDateStamp);
        }
        return false;
    }

    if (g_log) {
        g_log->info("BUILD_OK,base=0x{:016X},image_size=0x{:X},timestamp=0x{:08X}", g_exeBase, g_imageSize,
                    nt->FileHeader.TimeDateStamp);
        g_log->info("EXPECTED_SHA256,085c2f82d1c963c40b3d2d55786661dfee2b18cbbf388a710c00fa76c5e9bb45");
        g_log->info(
            ...
            g_exeBase + kRvaRegisteredSourceTablePtr, kRegisteredSourceRecordSize, kRegisteredSourceCount);
    }
    return true;
}
...
bool InstallOne(std::uintptr_t rva, void* hook, void** original, const char* name) {
    ...
    if (!IsInsideExe(targetValue)) {
        if (g_log)
            g_log->error("HOOK_RANGE_FAIL,{},rva=0x{:X}", name, rva);
        return false;
    }
    ...
    if (created != MH_OK && created != MH_ERROR_ALREADY_CREATED) {
        if (g_log)
            g_log->error("HOOK_CREATE_FAIL,{},status={}", name, static_cast<int>(created));
        return false;
    }
    const auto enabled = MH_EnableHook(target);
    if (enabled != MH_OK && enabled != MH_ERROR_ENABLED) {
        if (g_log)
            g_log->error("HOOK_ENABLE_FAIL,{},status={}", name, static_cast<int>(enabled));
        return false;
    }
    if (g_log)
        g_log->info("HOOK_OK,{},address=0x{:016X}", name, targetValue);
    return true;
}
...
Buffer040 ReadBuffer040(std::uintptr_t h, bool identity = true) {
    ...
    if (!h)
        return b;
    if (SafeReadValue(h, b.pcm))
        b.reads |= 1;
    if (SafeReadValue(h + 8, b.mask))
        b.reads |= 2;
    if (SafeReadValue(h + 0x10, b.stride))
        b.reads |= 4;
    if (SafeReadValue(h + 0x12, b.frames))
        b.reads |= 8;
    if (!identity)
        return b;
    if (SafeReadValue(h + 0x40, b.node))
        b.reads |= 16;
    if (b.node && SafeReadValue(b.node + 0x10, b.wrapper))
        b.reads |= 32;
    if (b.wrapper && SafeReadValue(b.wrapper + 0x18, b.context))
        b.reads |= 64;
    if (b.context && SafeReadValue(b.context + 0x70, b.playing))
        b.reads |= 128;
    if (b.context && SafeReadValue(b.context + 0xA8, b.a8))
        b.reads |= 256;
    if (b.context && SafeReadValue(b.context + 0x148, b.a148))
        b.reads |= 512;
    return b;
}
...
void Queue040(Event040 e) {
    ...
    e.seq = g_seq040.fetch_add(1) + 1;
    if (g_queueLock040.test_and_set(std::memory_order_acquire)) {
        ++g_drop040;
        return;
    }
    if (g_count040 == kQueue040)
        ++g_drop040;
    else {
        g_queue040[g_head040] = e;
        g_head040 = (g_head040 + 1) % kQueue040;
        ++g_count040;
    }
...
struct Scope040 {
    Event040 e{};
    std::uintptr_t src = 0, dst = 0;
    bool enabled = false;

    Scope040(std::uint32_t kind, std::uintptr_t caller, std::uintptr_t owner, std::uintptr_t source = 0,
             std::uintptr_t destination = 0)
        : src(source), dst(destination) {
        if (!g_capture040.load(std::memory_order_relaxed))
            return;
        if (g_budget040.fetch_add(1) >= 100000) {
        ...
        e.caller = caller;
        e.owner = owner;
        e.session = g_session040.load();
        e.call = g_call040.fetch_add(1) + 1;
        e.group = g_outputGroup041;
    }
    ...
    void Emit(std::uint32_t phase) {
        if (!enabled)
            return;
        e.phase = phase;
        e.src = ReadBuffer040(src, e.kind == 1 || e.kind == 2);
        e.dst = ReadBuffer040(dst, false);
        Queue040(e);
    }
};
...
void Drain040() {
    ...
    for (unsigned i = 0; i < 4096; ++i) {
        ...
        if (found) {
            e = g_queue040[g_tail040];
            g_tail040 = (g_tail040 + 1) % kQueue040;
            --g_count040;
        }
        g_queueLock040.clear(std::memory_order_release);
        if (!found)
            break;
        g_log->info(
            "B41,seq={},call={},session={},qpc={},tid={},kind={},phase={},caller=0x{:X},owner=0x{:X},body=0x{:X},matches={},group={}",
            e.seq, e.call, e.session, e.tick, e.tid, e.kind, e.phase, e.caller, e.owner, e.body, e.matches,
            e.group);
        for (unsigned side = 0; side < 2; ++side) {
            ...
            if (!b.header)
                continue;
            g_log->info(
                "B41_BUFFER,seq={},side={},header=0x{:X},pcm=0x{:X},mask={},stride={},frames={},reads={},node=0x{:X},wrapper=0x{:X},context=0x{:X},playing={},a8=0x{:X},a148=0x{:X}",
                e.seq, side, b.header, b.pcm, b.mask, b.stride, b.frames, b.reads, b.node, b.wrapper,
                b.context, b.playing, b.a8, b.a148);
        }
        if (e.kind >= 4)
            g_log->info(
                "B41_SINK,seq={},input=0x{:X},ring=0x{:X},gate=0x{:X},channels={},ready={},write={},read={},frames={},processed={},bytes={},result={}",
                e.seq, e.input, e.ring, e.gate, e.channels, e.ready, e.write, e.read, e.frames, e.processed,
                e.bytes, e.result);
    }
}
...
void Control040() {
    ...
    if (down && !prior) {
        if (g_capture040.load())
            g_capture040.store(false);
        else if (!reported) {
            ++g_session040;
            g_budget040.store(0);
            until = now + 20000;
            g_capture040.store(true);
            reported = true;
            LARGE_INTEGER hz{};
            QueryPerformanceFrequency(&hz);
            if (g_log)
                g_log->info("B41_START,session={},qpc_hz={},duration_ms=20000", g_session040.load(),
                            hz.QuadPart);
        }
    }
    prior = down;
    if (reported && now >= until)
        g_capture040.store(false);
    Drain040();
    if (reported && !g_capture040.load()) {
        if (g_log) {
            g_log->info("B41_STOP,session={},calls={},queue_drops_total={}", g_session040.load(),
                        g_budget040.load(), g_drop040.load());
            g_log->flush();
        }
        reported = false;
    }
...
void ReadSink040(Event040& e, std::uintptr_t sink, bool retainRing) {
    SafeReadValue(sink + 0x20, e.input);
    SafeReadValue(sink + 0x32, e.frames);
    SafeReadValue(sink + 0x58, e.channels);
    SafeReadValue(sink + 0x80, e.write);
    SafeReadValue(sink + 0x82, e.read);
    SafeReadValue(sink + 0xC8, e.ready);
    SafeReadValue(sink + 8, e.processed);
    SafeReadValue(g_exeBase + 0x2B9FA28, e.gate);
    if (!retainRing && e.write < 8)
        SafeReadValue(sink + 0x88 + 8 * e.write, e.ring);
}

std::uint64_t RunProducer040(std::int64_t sink, Producer040 original, std::uint32_t kind,
                             std::uintptr_t caller) {
    Scope040 t(kind, caller, static_cast<std::uintptr_t>(sink));
    if (t.enabled) {
        ReadSink040(t.e, t.e.owner, false);
        t.Emit(0);
    }
    const auto result = original(sink);
    if (t.enabled) {
        ReadSink040(t.e, t.e.owner, true);
        t.Emit(1);
    }
    return result;
}
...
std::uint64_t __fastcall TransferHook041(std::uintptr_t bus, std::uintptr_t destinationHeader) {
    Scope040 t(7, reinterpret_cast<std::uintptr_t>(_ReturnAddress()), bus, bus ? bus + 0x420 : 0,
               destinationHeader);
    t.Emit(0);
...
void __fastcall OutputStageHook041(std::uintptr_t entry) {
    ...
    if (t.enabled) {
        SafeReadValue(entry, bus);
        SafeReadValue(entry + 8, sink);
        t.src = bus ? bus + 0x420 : 0;
        t.dst = sink ? sink + 0x20 : 0;
        t.e.group = t.e.call;
        g_outputGroup041 = t.e.call;
        if (sink)
            ReadSink040(t.e, sink, false);
        t.Emit(0);
    }
    g_outputStage041(entry);
    if (t.enabled) {
        if (sink)
            ReadSink040(t.e, sink, true);
        t.Emit(1);
    }
...
void RememberSink039(std::uintptr_t sink) {
    for (auto& slot : g_observedSinks039) {
        ...
        if (current == sink)
            return;
        if (current == 0 && slot.compare_exchange_strong(current, sink))
            return;
    }
}

bool IsOurRingBuffer039(void* voice, const SubmitBuffer039& b, const void* wma) {
    if (wma || b.audioBytes == 0 || b.audioBytes > sizeof(g_silence039) || b.audioData == nullptr ||
        b.flags != 0 || b.playBegin != 0 || b.playLength != 0 || b.loopBegin != 0 || b.loopLength != 0 ||
        b.loopCount != 0)
        return false;
    for (auto& slot : g_observedSinks039) {
        ...
        if (!SafeReadValue(sink, vt) || vt != g_exeBase + 0x21358F0ull ||
            !SafeReadValue(sink + 0x70, sourceVoice) ||
            sourceVoice != reinterpret_cast<std::uintptr_t>(voice) || !SafeReadValue(sink + 0x58, channels) ||
            channels == 0 || channels > 8 || !SafeReadValue(sink + 0xCC, blockAlign) ||
            blockAlign != channels * sizeof(float) ||
            b.audioBytes != static_cast<std::uint32_t>(blockAlign) * 1024u)
            continue;
        for (unsigned i = 0; i < 8; ++i) {
            std::uintptr_t pcm = 0;
            if (SafeReadValue(sink + 0x88 + i * 8u, pcm) &&
                pcm == reinterpret_cast<std::uintptr_t>(b.audioData))
                return true;
        }
    }
    return false;
...
bool SamplePeak039(const unsigned char* pcm, std::uint32_t bytes, std::uint32_t* peakMicro) {
    if (!IsReadable(pcm, bytes) || bytes < sizeof(float))
        return false;
    __try {
...
long __fastcall SubmitHook039(void* voice, const SubmitBuffer039* input, const void* wma) {
    ...
    if (eligible) {
        ...
        ++g_submitOther039;
    Scope040 t(6, reinterpret_cast<std::uintptr_t>(_ReturnAddress()),
               reinterpret_cast<std::uintptr_t>(voice));
    t.e.input = reinterpret_cast<std::uintptr_t>(local.audioData);
    t.e.bytes = local.audioBytes;
    t.e.matches = eligible ? 1u : 0u;
    t.Emit(0);
    ...
    if (eligible) {
        ...
        if (result < 0)
            g_submitFailed039.fetch_add(1, std::memory_order_relaxed);
    }
    return result;
}
...
void EnsureSubmitHook039() {
    if (g_submitHookState039.load(std::memory_order_acquire) != 0)
        return;
    for (auto& slot : g_observedSinks039) {
        const auto sink = slot.load(std::memory_order_acquire);
        if (!sink)
        ...
        if (!SafeReadValue(sink, vt) || vt != g_exeBase + 0x21358F0ull ||
            !SafeReadValue(sink + 0x70, voice) || !voice || !SafeReadValue(voice, voiceVt) || !voiceVt ||
            !SafeReadValue(voiceVt + 0xA8, target) || !target)
            continue;
        MEMORY_BASIC_INFORMATION mbi{};
        ...
        if (protect != PAGE_EXECUTE && protect != PAGE_EXECUTE_READ && protect != PAGE_EXECUTE_READWRITE &&
            ...
            continue;
        int expected = 0;
        if (!g_submitHookState039.compare_exchange_strong(expected, 1))
            return;
        ...
        if (g_log)
            g_log->info(
                "SUBMIT_DISCOVERY,sink=0x{:X},voice=0x{:X},voice_vtable=0x{:X},target=0x{:X},module={}", sink,
                voice, voiceVt, target, leaf ? leaf + 1 : module);
        ...
        if (created != MH_OK || !trampoline) {
            if (g_log) {
                g_log->error("SUBMIT_HOOK_CREATE_FAILED,status={}", static_cast<int>(created));
                g_log->flush();
            }
        ...
        if (g_log) {
            g_log->info("SUBMIT_HOOK_INSTALL,state={},enable_status={}", enabled == MH_OK ? 2 : -1,
...
void LogSubmitSummary039() {
    if (!g_log)
        return;
    g_log->info(
        "SUBMIT_SUMMARY,hook_state={},calls={},eligible={},other={},mute_attempts={},mute_accepted={},submit_failed={},mute_input_nonzero_blocks={},mute_input_peak_micro={},pcm_probe_failed={},last_bytes={},last_result=0x{:08X},last_voice=0x{:X},gate_zero_calls={},gate_nonzero_calls={},gate_unreadable_calls={}",
        g_submitHookState039.load(), g_submitCalls039.load(), g_submitEligible039.load(),
        g_submitOther039.load(), g_muteAttempts039.load(), g_muteAccepted039.load(), g_submitFailed039.load(),
        g_muteEnergy039.load(), g_mutePeakMicro039.load(), g_probeBadPcm039.load(), g_lastBytes039.load(),
        g_lastResult039.load(), g_lastVoice039.load(), g_gateZero039.load(), g_gateNonzero039.load(),
        g_gateUnreadable039.load());
}
...
std::uintptr_t RegisteredTableBase() {
    ...
    return *reinterpret_cast<const std::uintptr_t*>(g_exeBase + kRvaRegisteredSourceTablePtr);
}

bool InspectRecordCandidate(std::uint32_t source, std::uintptr_t& tableBase, std::uintptr_t& candidate,
                            std::uint32_t& storedSource, std::uint64_t& storedKey) {
    tableBase = 0;
    candidate = 0;
    storedSource = 0;
    storedKey = 0;
    if (source == 0 || g_exeBase == 0)
        return false;

    tableBase = RegisteredTableBase();
    if (tableBase == 0)
        return false;
    candidate = tableBase + static_cast<std::uintptr_t>(source & 0xffu) * kRegisteredSourceRecordSize;

    if (!IsReadable(reinterpret_cast<const void*>(candidate), 0x34))
        return false;
...
bool ResolveRecordForSource(std::uint32_t source, std::uintptr_t& record, std::uint64_t& objectKey) {
    record = 0;
    objectKey = 0;
    std::uintptr_t tableBase = 0;
...
std::uint32_t FindRecordsByObjectKey(std::uint64_t objectKey, std::uint32_t& source, std::uintptr_t& record) {
    source = 0;
    record = 0;
    if (objectKey == 0 || g_exeBase == 0)
        return 0;

    const auto base = RegisteredTableBase();
    if (base == 0 || !IsReadable(reinterpret_cast<const void*>(base),
                                 kRegisteredSourceCount * kRegisteredSourceRecordSize))
        return 0;

    std::uint32_t matches = 0;
    for (std::size_t i = 0; i < kRegisteredSourceCount; ++i) {
        const auto candidate = base + static_cast<std::uintptr_t>(i) * kRegisteredSourceRecordSize;
        std::uint32_t key = 0;
        std::uint32_t candidateSource = 0;
        std::memcpy(&key, reinterpret_cast<const void*>(candidate + 0x00), sizeof(key));
        if (key != static_cast<std::uint32_t>(objectKey))
            continue;
        std::memcpy(&candidateSource, reinterpret_cast<const void*>(candidate + 0x30),
                    sizeof(candidateSource));
...
RecentFoxSlot* FindFoxSlot(std::uint32_t source, bool create) {
    ...
    for (std::size_t probe = 0; probe < 8; ++probe) {
        ...
        if (existing == 0 && create) {
            ...
            if (slot.source.compare_exchange_strong(expected, source, std::memory_order_acq_rel,
                                                    std::memory_order_acquire)) {
                return &slot;
            }
            if (expected == source)
...
void RememberFoxEvent(std::uint32_t source, std::uintptr_t body, std::uint32_t eventId, std::uint64_t tick) {
...
bool LookupFox(std::uint32_t source, std::uintptr_t& body, std::uint32_t& eventId, std::uint64_t now) {
    body = 0;
    eventId = 0;
...
void RememberFoxTransform(std::uint32_t source, std::uintptr_t body, const std::uint32_t (&payload)[8],
                          std::uint64_t tick) {
    auto* slot = FindFoxSlot(source, true);
    if (!slot)
        return;
...
bool ShouldEmitTransform(std::uint32_t source, std::uint64_t now) {
    ...
    for (;;) {
        if (now >= prior && now - prior < kTransformLogIntervalMs)
            return false;
        if (slot->transformLogTick.compare_exchange_weak(prior, now, std::memory_order_relaxed,
    ...
    for (std::size_t probe = 0; probe < 8; ++probe) {
        ...
        if (existing == body) {
            ...
            for (;;) {
                if (now >= prior && now - prior < kTransformLogIntervalMs)
                    return false;
                if (slot.tick.compare_exchange_weak(prior, now, std::memory_order_relaxed,
                                                    std::memory_order_relaxed))
                    return true;
            }
        }
        if (existing == 0) {
            std::uintptr_t expected = 0;
            if (slot.body.compare_exchange_strong(expected, body, std::memory_order_acq_rel,
                                                  std::memory_order_acquire) ||
                expected == body) {
...
void RememberKey(std::uint64_t key, std::uint32_t eventId, std::uint32_t source, std::uintptr_t body,
                 std::uintptr_t record, std::uint32_t recordMatches, std::uint64_t tick) {
...
void UpdateKeyPostResult(std::uint64_t key, std::int32_t result) {
    auto* slot = FindKeySlot(key, false);
    if (slot)
        slot->postResult.store(result, std::memory_order_release);
}
...
PendingFoxPost* FindPendingFoxPost(std::uint32_t eventId) {
    for (std::size_t i = g_pendingFoxDepth; i > 0; --i) {
        ...
        if (pending.eventId == eventId && pending.body != 0)
            return &pending;
    }
    return nullptr;
}
...
bool ShouldTraceFirstLookup(std::uint64_t key, std::uintptr_t object) {
    ...
    if (!slot)
        return false;
    std::uintptr_t expected = 0;
    if (slot->lookupTraceObject.compare_exchange_strong(expected, object, std::memory_order_acq_rel,
                                                        std::memory_order_acquire)) {
...
RecentObjectSlot* FindObjectSlot(std::uintptr_t object, bool create) {
    ...
    for (std::size_t probe = 0; probe < 8; ++probe) {
        ...
        if (existing == 0 && create) {
            std::uintptr_t expected = 0;
            if (slot.object.compare_exchange_strong(expected, object, std::memory_order_acq_rel,
                                                    std::memory_order_acquire)) {
                return &slot;
            }
            if (expected == object)
...
void RememberObject(std::uintptr_t object, const IdentityMeta& meta, std::uint64_t tick) {
...
bool LookupObject(std::uintptr_t object, IdentityMeta& meta, std::uint64_t now) {
...
RecentPlayingSlot* FindPlayingSlot(std::uint32_t playingId, bool create) {
    ...
    for (std::size_t probe = 0; probe < 8; ++probe) {
        ...
        if (existing == playingId)
            return &slot;
        if (existing == 0 && create) {
            std::uint32_t expected = 0;
            if (slot.playingId.compare_exchange_strong(expected, playingId, std::memory_order_acq_rel,
                                                       std::memory_order_acquire)) {
                return &slot;
            }
...
void RememberPlaying(std::uint32_t playingId, const IdentityMeta& meta, std::uint64_t tick) {
...
bool LookupPlaying(std::uint32_t playingId, IdentityMeta& meta, std::uint64_t now) {
...
bool SameIdentity(const IdentityMeta& a, const IdentityMeta& b) {
    if (a.key == 0 || b.key == 0)
        return false;
    return a.key == b.key && a.eventId == b.eventId && a.source == b.source && a.body == b.body &&
           a.record == b.record;
}
...
bool PublishLatestListenerTransform(std::int64_t ownerValue) {
    ...
    if (owner == 0 || !IsReadable(reinterpret_cast<const void*>(owner + 0x60), sizeof(std::uintptr_t))) {
        return false;
    }

    std::uintptr_t listener = 0;
    std::memcpy(&listener, reinterpret_cast<const void*>(owner + 0x60), sizeof(listener));
    if (listener == 0 ||
        !IsReadable(reinterpret_cast<const void*>(listener + 0x40), 8 * sizeof(std::uint32_t))) {
        return false;
    }

    std::uint32_t payload[8]{};
    std::memcpy(payload, reinterpret_cast<const void*>(listener + 0x40), sizeof(payload));
    for (std::size_t i = 0; i < 8; ++i) {
        g_latestListenerTransformBits[i].store(payload[i], std::memory_order_relaxed);
    }
    g_latestListenerTransformTick.store(GetTickCount64(), std::memory_order_release);
    return true;
}

bool ReadLatestListenerTransform(std::uint32_t (&payload)[8]) {
    const auto tick = g_latestListenerTransformTick.load(std::memory_order_acquire);
    if (tick == 0)
        return false;
    const auto now = GetTickCount64();
    if (now > tick && now - tick > kListenerFreshMs)
        return false;

    for (std::size_t i = 0; i < 8; ++i) {
        payload[i] = g_latestListenerTransformBits[i].load(std::memory_order_relaxed);
    }
    return true;
}

bool ReadFoxTransformForIdentity(const IdentityMeta& meta, std::uint32_t (&payload)[8]) {
    if (meta.source == 0 || meta.body == 0)
        return false;
    auto* slot = FindFoxSlot(meta.source, false);
    if (!slot)
        return false;
    if (slot->body.load(std::memory_order_acquire) != meta.body)
        return false;
    if (slot->transformTick.load(std::memory_order_acquire) == 0)
        return false;

    for (std::size_t i = 0; i < 8; ++i) {
        payload[i] = slot->transformBits[i].load(std::memory_order_relaxed);
    }
    return true;
}

bool ComputeDirectGeometry(const IdentityMeta& meta, float& outAzimuthDeg, float& outElevationDeg,
                           float& outDistance, const float* nativeWorld050 = nullptr) {
    ...
    if (nativeWorld050) {
        for (unsigned i = 0; i < 3; ++i)
            source[4 + i] = FloatBits(nativeWorld050[i]);
    ...
    const double horizontal = std::sqrt(right * right + forward * forward);
    if (!std::isfinite(horizontal) || horizontal < 1.0e-9) {
        return false;
    }

    outAzimuthDeg = static_cast<float>(std::atan2(right, forward) * kRadiansToDegrees);
    outElevationDeg = static_cast<float>(std::atan2(localY, horizontal) * kRadiansToDegrees);
    outDistance = static_cast<float>(std::sqrt(dx * dx + dy * dy + dz * dz));
    return std::isfinite(outAzimuthDeg) && std::isfinite(outElevationDeg) && std::isfinite(outDistance);
}

BinauralHistoryState& GetBinauralHistory(std::uintptr_t node, std::uintptr_t destination) {
    // Hash both pointers to avoid collisions from regularly spaced render nodes.
    const std::uint64_t key = ((static_cast<std::uint64_t>(node >> 4) * 0x9E3779B97F4A7C15ull) ^
                               (static_cast<std::uint64_t>(destination >> 4) * 0xC2B2AE3D27D4EB4Full)) >>
                              54;
    ...
    for (std::size_t probe = 0; probe < 8; ++probe) {
        ...
        if (victim056 == nullptr || candidate.lastUse056 < victim056->lastUse056)
            victim056 = &candidate;
    }
...
void BuildStereoEarCoefficientBlocks(const float* original, float* left, float* right) {
    ...
    for (int pass = 0; pass < 2; ++pass) {
        ...
        const float centered = std::sqrt((l * l + r * r) * 0.5f);
...
void BuildInterpolatedMeasuredHrir360(float angleDeg, float* leftFilter, float* rightFilter) {
    while (angleDeg < -180.0f)
        angleDeg += 360.0f;
    while (angleDeg > 180.0f)
        angleDeg -= 360.0f;

    const float position = (angleDeg - KEMAR360::kMinAngleDeg) / KEMAR360::kStepDeg;
    std::uint32_t low = static_cast<std::uint32_t>(std::floor(position));
    if (low >= KEMAR360::kDirectionCount - 1) {
        low = static_cast<std::uint32_t>(KEMAR360::kDirectionCount - 1);
    }
    const std::uint32_t high = (low + 1u < KEMAR360::kDirectionCount) ? low + 1u : low;
    ...
    for (std::size_t tap = 0; tap < KEMAR360::kTapCount; ++tap) {
        leftFilter[tap] = KEMAR360::kHrir[low][0][tap] * oneMinus + KEMAR360::kHrir[high][0][tap] * alpha;
        rightFilter[tap] = KEMAR360::kHrir[low][1][tap] * oneMinus + KEMAR360::kHrir[high][1][tap] * alpha;
    }
}

// Interpolate azimuth within each ring, then between elevation rings.
void AccumulateRing059(std::size_t ring, float az360, float weight, float* leftFilter, float* rightFilter,
                       float emphasis) {
    if (weight <= 0.0f)
        return;
    const std::size_t off = HRIRSET::kRingOffset[ring], n = HRIRSET::kRingSize[ring];
    std::size_t j = n - 1, next = 0;
    float t = 0.0f;
    if (n > 1) {
        for (std::size_t k = 0; k < n; ++k) {
            if (HRIRSET::kAzimuth[off + k] <= az360)
                j = k;
            else
                break;
        }
    ...
    const float* l0 = HRIRSET::kHrir[off + j][0];
    const float* r0 = HRIRSET::kHrir[off + j][1];
    const float* l1 = HRIRSET::kHrir[off + next][0];
    const float* r1 = HRIRSET::kHrir[off + next][1];
    ...
    if (s > 0.0f) {
        ...
        const float* el0 = HRIRSET::kHrirEmphasis[off + j][0];
        const float* er0 = HRIRSET::kHrirEmphasis[off + j][1];
        const float* el1 = HRIRSET::kHrirEmphasis[off + next][0];
        const float* er1 = HRIRSET::kHrirEmphasis[off + next][1];
...
void BuildHrirWithEmphasis064(float azimuthDeg, float elevationDeg, float emphasis, float* leftFilter,
                              float* rightFilter);

void BuildInterpolatedMeasuredHrir3D(float azimuthDeg, float elevationDeg, float* leftFilter,
                                     float* rightFilter) {
    BuildHrirWithEmphasis064(azimuthDeg, elevationDeg, g_emphasis062.load(std::memory_order_relaxed),
                             leftFilter, rightFilter);
}

void BuildHrirWithEmphasis064(float azimuthDeg, float elevationDeg, float emphasis, float* leftFilter,
                              float* rightFilter) {
    float a = std::fmod(azimuthDeg, 360.0f);
    if (a < 0.0f)
        a += 360.0f;
    if (a >= 360.0f)
        a = 0.0f;
    if (!std::isfinite(elevationDeg))
        elevationDeg = 0.0f;
    constexpr std::size_t rings = HRIRSET::kRingCount;
    const float e =
        (std::min)(HRIRSET::kRingElevation[rings - 1], (std::max)(HRIRSET::kRingElevation[0], elevationDeg));
    std::size_t r0 = 0;
    while (r0 + 2 < rings && HRIRSET::kRingElevation[r0 + 1] <= e)
        ++r0;
...
bool PrepareDirectMeasuredHrir(const float* source, std::uint32_t frames, std::uintptr_t node,
                               std::uintptr_t destination, float azimuthDeg, bool pure046 = false,
                               float elevationDeg = 0.0f) {
    if (source == nullptr || frames == 0 || frames > static_cast<std::uint32_t>(kBinauralMaxFrames)) {
        return false;
    }
    for (std::uint32_t i = 0; i < frames; ++i) {
        if (!std::isfinite(source[i]))
            return false; // never feed NaN/inf into filter history
    }
    ...
    if (!state.initialized) {
        ...
        state.priorElevationDeg =
            (g_elevation059.load(std::memory_order_relaxed) && std::isfinite(elevationDeg))
                ? (std::min)(90.0f, (std::max)(-90.0f, elevationDeg))
                : 0.0f;
    ...
    while (smoothedAngle > 180.0f)
        smoothedAngle -= 360.0f;
    while (smoothedAngle < -180.0f)
        smoothedAngle += 360.0f;
    ...
    const float targetElevation =
        g_elevation059.load(std::memory_order_relaxed) && std::isfinite(elevationDeg)
            ? (std::min)(90.0f, (std::max)(-90.0f, elevationDeg))
            : 0.0f;
    const float smoothedElevation =
        state.priorElevationDeg + (targetElevation - state.priorElevationDeg) * 0.35f;
    if (smoothedElevation < -20.0f)
        g_elevBelow059.fetch_add(1, std::memory_order_relaxed);
    else if (smoothedElevation > 20.0f)
        g_elevAbove059.fetch_add(1, std::memory_order_relaxed);
    else
        g_elevLevel059.fetch_add(1, std::memory_order_relaxed);

    alignas(16) float leftFilter[KEMAR360::kTapCount];
    alignas(16) float rightFilter[KEMAR360::kTapCount];
    BuildInterpolatedMeasuredHrir3D(smoothedAngle, smoothedElevation, leftFilter, rightFilter);
    ...
    while (angleStep > 180.0f)
        angleStep -= 360.0f;
    while (angleStep < -180.0f)
        angleStep += 360.0f;
    const bool crossfade056 =
        std::fabs(angleStep) > 0.01f || std::fabs(smoothedElevation - state.priorElevationDeg) > 0.01f;
    if (crossfade056) {
        BuildInterpolatedMeasuredHrir3D(state.priorAzimuthDeg, state.priorElevationDeg, priorLeftFilter,
                                        priorRightFilter);
        g_filterCrossfades056.fetch_add(1, std::memory_order_relaxed);
    }
    const float invFrames056 = 1.0f / static_cast<float>(frames);

    constexpr std::uint32_t historyMask = static_cast<std::uint32_t>(kBinauralHistorySamples - 1);
    ...
    for (std::uint32_t tap = 0; tap < static_cast<std::uint32_t>(KEMAR360::kTapCount); ++tap) {
    ...
    auto slewDelay = [](std::int32_t& prior, std::uint32_t want) {
        if (prior < 0) {
            prior = static_cast<std::int32_t>(want);
        } else if (static_cast<std::uint32_t>(prior) < want)
            ++prior;
        else if (static_cast<std::uint32_t>(prior) > want)
            --prior;
    ...
    for (std::uint32_t i = 0; i < frames; ++i) {
        ...
        if (crossfade056) {
            ...
            for (std::size_t tap = 0; tap < KEMAR360::kTapCount; ++tap) {
                const float x = state.history[(readIndex - static_cast<std::uint32_t>(tap)) & historyMask];
        ...
        const float delayedDryLeft = state.history[(readIndex - leftDelay) & historyMask];
        const float delayedDryRight = state.history[(readIndex - rightDelay) & historyMask];

        state.lowHrtfLeft += kHybridLowpassAlpha * (left - state.lowHrtfLeft);
        state.lowHrtfRight += kHybridLowpassAlpha * (right - state.lowHrtfRight);
        state.lowDryLeft += kHybridLowpassAlpha * (delayedDryLeft - state.lowDryLeft);
        state.lowDryRight += kHybridLowpassAlpha * (delayedDryRight - state.lowDryRight);

        const float hybridLeft = (left - state.lowHrtfLeft) + state.lowDryLeft;
        const float hybridRight = (right - state.lowHrtfRight) + state.lowDryRight;
        ...
        blockPeak =
            (std::max)(blockPeak, (std::max)(std::fabs(g_directLeftEar[i]), std::fabs(g_directRightEar[i])));

        state.writeIndex = (state.writeIndex + 1u) & historyMask;
    }
...
bool PrepareWideWorldMono(std::uintptr_t sourceBuffer, std::uint32_t sourceMask,
                          const float* coefficientBlock, std::uint32_t frames, float* monoCoefficients,
                          float& sourcePeak) {
    if (sourceBuffer == 0 || coefficientBlock == nullptr || monoCoefficients == nullptr || frames == 0 ||
    ...
    const std::uint32_t channels = static_cast<std::uint32_t>(__popcnt(sourceMask));
    if (channels != 2u && channels != 4u) {
        return false;
    }

    // Validate every source-channel coefficient row.
    if (!IsReadable(coefficientBlock, static_cast<std::size_t>(channels) * 16u * sizeof(float))) {
        return false;
    }

    std::uintptr_t sourceData = 0;
    std::uint16_t maxFrames16 = 0;
    if (!SafeReadValue(sourceBuffer + 0x00, sourceData) || !SafeReadValue(sourceBuffer + 0x10, maxFrames16)) {
        return false;
    }

    const std::uint32_t maxFrames = maxFrames16;
    if (sourceData == 0 || maxFrames < frames || maxFrames > static_cast<std::uint32_t>(kBinauralMaxFrames)) {
        return false;
    }

    const std::size_t totalSamples = static_cast<std::size_t>(channels) * maxFrames;
    if (!IsReadable(reinterpret_cast<const void*>(sourceData), totalSamples * sizeof(float))) {
        return false;
    }
    ...
    for (std::uint32_t i = 0; i < frames; ++i) {
        ...
        for (std::uint32_t ch = 0; ch < channels; ++ch) {
            const float x = base[static_cast<std::size_t>(ch) * maxFrames + i];
            if (!std::isfinite(x))
                return false;
            sum += x;
    ...
    for (int half : {0, 8}) {
        ...
        for (std::uint32_t ch = 0; ch < channels; ++ch) {
            const float* row = coefficientBlock + static_cast<std::size_t>(ch) * 16u;
...
void RecordEarMetrics044(const float* input, std::uint32_t frames, bool bypass) {
    double in = 0, left = 0, right = 0;
    float ip = 0, lp = 0, rp = 0;
    for (std::uint32_t i = 0; i < frames; ++i) {
        const float x = input[i], l = g_directLeftEar[i], r = g_directRightEar[i];
        in += double(x) * x;
        left += double(l) * l;
        right += double(r) * r;
        ip = (std::max)(ip, std::fabs(x));
        lp = (std::max)(lp, std::fabs(l));
        rp = (std::max)(rp, std::fabs(r));
    }
    if (!frames)
        return;
    g_inputPeak044.store(FloatBits(ip));
    g_leftPeak044.store(FloatBits(lp));
    g_rightPeak044.store(FloatBits(rp));
...
bool ApplyWideWorldHrtf(std::int64_t destinationValue, std::int64_t sourceBufferValue,
                        const float* coefficientBlock, std::uintptr_t node, std::uintptr_t destination,
                        std::uint32_t sourceMask, const IdentityMeta& meta, float& sourcePeak,
                        float& leftPeak, float& rightPeak, float& azimuthDeg, float& elevationDeg,
                        float& distance, std::uint32_t& framesOut, bool bypass044 = false,
                        bool pure046 = false, const float* nativeWorld050 = nullptr) {
    if (!bypass044 && !ComputeDirectGeometry(meta, azimuthDeg, elevationDeg, distance, nativeWorld050)) {
        return false;
    }
    ...
    const std::uint32_t frames = validFrames16 != 0 ? validFrames16 : maxFrames16;
    if (originalData == 0 || frames == 0 || frames > static_cast<std::uint32_t>(kBinauralMaxFrames)) {
        g_directHrtfBadPcm.fetch_add(1, std::memory_order_relaxed);
        return false;
    }

    const unsigned rows047 = sourceMask == 4 ? 1u : (sourceMask == 3 ? 2u : (sourceMask == 0x33 ? 4u : 0u));
    if (!rows047 || !IsReadable(coefficientBlock, rows047 * 16u * sizeof(float)))
        return false;
    for (unsigned i = 0; i < rows047 * 16u; ++i)
        if (!std::isfinite(coefficientBlock[i]))
            return false;
    ...
    if (sourceMask == 0x00000004u) {
        if (!IsReadable(coefficientBlock, 16u * sizeof(float)) ||
            !IsReadable(reinterpret_cast<const void*>(originalData),
                        static_cast<std::size_t>(frames) * sizeof(float))) {
            g_directHrtfBadPcm.fetch_add(1, std::memory_order_relaxed);
            return false;
        }

        std::memcpy(monoCoefficients, coefficientBlock, sizeof(monoCoefficients));
        hrtfSource = reinterpret_cast<const float*>(originalData);
        for (std::uint32_t i = 0; i < frames; ++i) {
            sourcePeak = (std::max)(sourcePeak, std::fabs(hrtfSource[i]));
            if (!std::isfinite(hrtfSource[i]))
                return false;
        }
    } else {
        multiSource = true;
        g_wideHrtfMultiAttempts.fetch_add(1, std::memory_order_relaxed);
        if (!PrepareWideWorldMono(sourceBuffer, sourceMask, coefficientBlock, frames, monoCoefficients,
                                  sourcePeak)) {
    ...
    if (bypass044) {
        ...
        if (!SafeReadValue(sourceBuffer + 0x10, maxFrames056) || maxFrames056 < frames)
            return false;
        ...
        for (std::uint32_t i = 0; i < frames; ++i)
            half056[i] = channel0[i] * 0.5f;
        if (!PrepareDirectMeasuredHrir(half056, frames, node, destination, azimuthDeg - spreadDeg056, pure046,
                                       elevationDeg)) {
            g_directHrtfBadPcm.fetch_add(1, std::memory_order_relaxed);
            return false;
        }
        ...
        for (std::uint32_t i = 0; i < frames; ++i)
            half056[i] = channel1[i] * 0.5f;
        if (!PrepareDirectMeasuredHrir(half056, frames, node, destination ^ 0x4000000000000000ull,
                                       azimuthDeg + spreadDeg056, pure046, elevationDeg)) {
            g_directHrtfBadPcm.fetch_add(1, std::memory_order_relaxed);
            return false;
        }
        for (std::uint32_t i = 0; i < frames; ++i) {
    ...
    } else if (!PrepareDirectMeasuredHrir(hrtfSource, frames, node, destination, azimuthDeg, pure046,
                                          elevationDeg)) {
        g_directHrtfBadPcm.fetch_add(1, std::memory_order_relaxed);
        return false;
    }
    ...
    if (bypass044)
        BuildBypassEarCoefficients044(monoCoefficients, leftCoefficients, rightCoefficients);
    else
        BuildStereoEarCoefficientBlocks(monoCoefficients, leftCoefficients, rightCoefficients);
    RecordEarMetrics044(hrtfSource, frames, bypass044);
    if (pure046)
        g_metricsMode044.store(4);

    for (std::uint32_t i = 0; i < frames; ++i) {
        leftPeak = (std::max)(leftPeak, std::fabs(g_directLeftEar[i]));
        rightPeak = (std::max)(rightPeak, std::fabs(g_directRightEar[i]));
    }
    ...
    if (!bypass044 && g_peakGuard066.load(std::memory_order_relaxed)) {
        const float originalGain =
            (std::max)((std::max)(std::fabs(monoCoefficients[0]), std::fabs(monoCoefficients[1])),
                       (std::max)(std::fabs(monoCoefficients[8]), std::fabs(monoCoefficients[9])));
        const float originalPeak = sourcePeak * originalGain;
        const float binauralPeak =
            (std::max)(leftPeak * (std::max)(leftCoefficients[0], leftCoefficients[8]),
        ...
        if (target > guardState.peakGuard066)
            target = (std::min)(target, guardState.peakGuard066 + 0.02f);
    ...
    if (!IsReadable(reinterpret_cast<const void*>(sourceBuffer), fakeBuffer.size())) {
        g_directHrtfBadPcm.fetch_add(1, std::memory_order_relaxed);
        return false;
    }
    std::memcpy(fakeBuffer.data(), reinterpret_cast<const void*>(sourceBuffer), fakeBuffer.size());

    const auto fakePtr = reinterpret_cast<std::uintptr_t>(fakeBuffer.data());
    *reinterpret_cast<std::uint32_t*>(fakeBuffer.data() + 0x08) = 0x00000004u;

    *reinterpret_cast<std::uintptr_t*>(fakeBuffer.data() + 0x00) =
        reinterpret_cast<std::uintptr_t>(g_directLeftEar);
    g_sourceToDestinationMixOriginal(destinationValue, static_cast<std::int64_t>(fakePtr), leftCoefficients);
    g_directHrtfLeftCalls.fetch_add(1, std::memory_order_relaxed);

    *reinterpret_cast<std::uintptr_t*>(fakeBuffer.data() + 0x00) =
        reinterpret_cast<std::uintptr_t>(g_directRightEar);
    g_sourceToDestinationMixOriginal(destinationValue, static_cast<std::int64_t>(fakePtr), rightCoefficients);
...
void RenewVerifiedIdentity(std::uint32_t playingId, std::uintptr_t object, const IdentityMeta& meta,
                           std::uint64_t now) {
    if (playingId != 0) {
...
bool ShouldEmitMatrixBind(std::uintptr_t context, std::uint64_t now) {
    ...
    for (std::size_t probe = 0; probe < 8; ++probe) {
        auto& slot = g_matrixBindGate[(start + probe) & (kMatrixBindGateSlots - 1)];
        auto existing = slot.context.load(std::memory_order_acquire);
        if (existing == context) {
            auto prior = slot.tick.load(std::memory_order_relaxed);
            for (;;) {
                if (now >= prior && now - prior < kMatrixBindLogIntervalMs)
                    return false;
                if (slot.tick.compare_exchange_weak(prior, now, std::memory_order_relaxed,
                                                    std::memory_order_relaxed))
                    return true;
...
bool IsRenderResearchEvent(std::uint32_t eventId) {
    return eventId == kEventGenerator || eventId == kEventSteam || eventId == kEventObservedA54F;
}
...
bool ShouldCaptureRenderContext(std::uintptr_t context, std::uint64_t now) {
    ...
    for (std::size_t probe = 0; probe < 8; ++probe) {
        ...
        if (existing == context) {
            auto prior = slot.tick.load(std::memory_order_relaxed);
            for (;;) {
                if (now >= prior && now - prior < kRenderSnapshotIntervalMs)
                    return false;
                if (slot.tick.compare_exchange_weak(prior, now, std::memory_order_relaxed,
                                                    std::memory_order_relaxed))
                    return true;
...
std::uint32_t CopyRenderBytes(std::uintptr_t base, std::array<std::uint8_t, N>& out) {
...
void EnqueueRenderSnapshot(const RenderSnapshot& snapshot) {
    const auto sequence = g_renderSnapshotWrite.fetch_add(1, std::memory_order_relaxed) + 1;
...
std::uint32_t FindRecentPlayingForIdentity(const IdentityMeta& meta, std::uint64_t now) {
    ...
    for (const auto& slot : g_recentPlaying) {
        ...
        if (tick == 0 || (now > tick && now - tick > kRecentIdentityTtlMs))
            continue;
        if (slot.eventId.load(std::memory_order_relaxed) != meta.eventId)
            continue;
        if (slot.key.load(std::memory_order_relaxed) != meta.key)
            continue;
        const auto body = slot.body.load(std::memory_order_relaxed);
        if (meta.body != 0 && body != meta.body)
            continue;
        if (tick >= bestTick) {
            bestTick = tick;
            bestPlaying = playing;
...
bool LogRenderIdentityMatch(const RenderSnapshot& snap, std::uint32_t region, std::uint32_t offset,
                            const char* matchType, std::uint64_t rawValue, std::uint32_t playingId,
                            const IdentityMeta& meta) {
    if (meta.eventId != snap.eventId || meta.body == 0)
        return false;
    if (playingId == 0) {
    ...
    if (g_log && CanLog()) {
        g_log->info("RENDER_ID_MATCH,tick={},event=0x{:08X},region={},offset=0x{:03X},"
                    ...
                    snap.tick, snap.eventId, RenderRegionName(region), offset, matchType, rawValue, playingId,
                    meta.key, static_cast<std::uint64_t>(meta.body), meta.source,
                    static_cast<std::uint64_t>(meta.record), static_cast<std::uint64_t>(snap.node),
                    static_cast<std::uint64_t>(snap.wrapper), static_cast<std::uint64_t>(snap.context),
                    static_cast<std::uint64_t>(snap.eventObject));
    }
    return true;
}

void ScanRenderRegion(const RenderSnapshot& snap, std::uint32_t region, const std::uint8_t* bytes,
                      std::size_t size, std::size_t& matches) {
    if (bytes == nullptr || size < 4 || matches >= 16)
        return;
    const auto now = GetTickCount64();
    ...
    for (std::size_t off = 0; off + 4 <= size && matches < 16; off += 4) {
        ...
        if (v32 != 0) {
            ...
            if (LookupPlaying(v32, meta, now) &&
                LogRenderIdentityMatch(snap, region, static_cast<std::uint32_t>(off), "playing32", v32, v32,
                                       meta)) {
                ++matches;
            }
            ...
            if (LookupKey(static_cast<std::uint64_t>(v32), meta, now) &&
                LogRenderIdentityMatch(snap, region, static_cast<std::uint32_t>(off), "key32", v32,
                                       meta.postResult > 0 ? static_cast<std::uint32_t>(meta.postResult) : 0u,
                                       meta)) {
                ++matches;
            }
        }
            ...
            if (v64 != 0) {
                ...
                if (LookupKey(v64, meta, now) &&
                    LogRenderIdentityMatch(
                        snap, region, static_cast<std::uint32_t>(off), "key64", v64,
                        meta.postResult > 0 ? static_cast<std::uint32_t>(meta.postResult) : 0u, meta)) {
                    ++matches;
                }
                ...
                if (LookupObject(static_cast<std::uintptr_t>(v64), meta, now) &&
                    LogRenderIdentityMatch(
                        snap, region, static_cast<std::uint32_t>(off), "wwise_object", v64,
                        meta.postResult > 0 ? static_cast<std::uint32_t>(meta.postResult) : 0u, meta)) {
                    ++matches;
                }
                ...
                if (hi == snap.eventId && LookupPlaying(lo, meta, now) &&
                    LogRenderIdentityMatch(snap, region, static_cast<std::uint32_t>(off),
    ...
    while (read < write && processed < kMaxRenderFlushPerBridge) {
        ...
        if (matches == 0) {
            ...
            if (n < 24 && g_log && CanLog()) {
                g_log->info("RENDER_SCAN_NO_MATCH,tick={},event=0x{:08X},node=0x{:016X},"
                            "wrapper=0x{:016X},context=0x{:016X},event_object=0x{:016X},"
                            "node_bytes={},wrapper_bytes={},context_bytes={}",
                            snap.tick, snap.eventId, static_cast<std::uint64_t>(snap.node),
                            static_cast<std::uint64_t>(snap.wrapper),
                            static_cast<std::uint64_t>(snap.context),
                            static_cast<std::uint64_t>(snap.eventObject), snap.nodeBytes, snap.wrapperBytes,
                            snap.contextBytes);
            }
        }
...
struct Census049 {
    ...
    std::uint64_t objectKey = 0;
    std::uint32_t kind = 0, event = 0, playing = 0, source = 0, routes = 0, matches = 0, srcMasks = 0,
                  dstMasks = 0;
...
void ObserveCensus049(Census049 x) {
    if (!g_debugLog063.load(std::memory_order_relaxed))
        return; // v0.63
    if (g_censusLock049.test_and_set(std::memory_order_acquire)) {
        ++g_censusBusy049;
        return;
    }
    const auto hash = x.key1 * 0x9E3779B97F4A7C15ull ^ x.key2 * 0xBF58476D1CE4E5B9ull ^
                      x.key3 * 0x94D049BB133111EBull ^ x.kind;
    bool stored = false;
    for (unsigned n = 0; n < 128; ++n) {
        ...
        if (CensusSame049(c, x)) {
            ...
            c.routes |= x.routes;
            c.matches |= x.matches;
            c.srcMasks |= x.srcMasks;
            c.dstMasks |= x.dstMasks;
            c.playingStages |= x.playingStages;
            c.objectStages |= x.objectStages;
            if (x.body)
                c.body = x.body;
            if (x.event)
                c.event = x.event;
            if (x.source)
                c.source = x.source;
            if (x.objectKey)
                c.objectKey = x.objectKey;
            if (x.kind == 4)
                for (unsigned i = 0; i < 3; ++i)
                    c.xyzBits050[i] = x.xyzBits050[i];
            stored = true;
            break;
        }
    }
...
void FlushCensus049(bool final = false) {
    ...
    if (flushing.test_and_set(std::memory_order_acquire))
        return;
    ...
    for (std::size_t i = 0; i < limit; ++i) {
        ...
        if (g_censusLock049.test_and_set(std::memory_order_acquire))
            break;
        auto& c = g_census049[cursor];
        cursor = (cursor + 1) & (kCensus049 - 1);
        if (c.kind && c.calls != c.emittedCalls && g_censusRows049 < 200000 &&
            (final || !c.emittedCalls || now - c.emittedAt >= 60000 ||
             (now >= c.last && now - c.last >= 5000))) {
            out = c;
            c.emittedCalls = c.calls;
            c.emittedAt = now;
            emit = true;
        }
        g_censusLock049.clear(std::memory_order_release);
        if (emit) {
            ++g_censusRows049;
            g_log->info(
                "SOURCE049,kind={},a=0x{:X},b=0x{:X},c=0x{:X},first={},last={},calls={},applied={},event=0x{:X},playing={},key=0x{:X},body=0x{:X},source=0x{:X},node=0x{:X},object=0x{:X},caller=0x{:X},routes={},match_bits={},src_masks_or={},dst_masks_or={},playing_stage_bits={},object_stage_bits={}",
                out.kind, out.key1, out.key2, out.key3, out.first, out.last, out.calls, out.applied,
                out.event, out.playing, out.objectKey, out.body, out.source, out.node, out.object, out.caller,
                out.routes, out.matches, out.srcMasks, out.dstMasks, out.playingStages, out.objectStages);
            if (out.kind == 4)
                g_log->info("POSITION050,a=0x{:X},b=0x{:X},c=0x{:X},key=0x{:X},x_bits={},y_bits={},z_bits={}",
                            out.key1, out.key2, out.key3, out.objectKey, out.xyzBits050[0], out.xyzBits050[1],
                            out.xyzBits050[2]);
        }
    }
    ...
    if (!final && down && !priorMarker) {
        g_log->info("MARK049,number={},tick={}", ++marker, now);
        g_log->flush();
    }
    priorMarker = down;
    if (final || now - summary >= 60000) {
        g_log->info("CENSUS049,tick={},unique={},rows={},busy_drops={},table_drops={},row_cap={},final={}",
                    now, g_censusUnique049.load(), g_censusRows049, g_censusBusy049.load(),
                    g_censusFull049.load(), g_censusRows049 >= 200000 ? 1 : 0, final ? 1 : 0);
        g_log->flush();
        summary = now;
    }
}
...
void EnqueueTrace(const TracePayload& p) {
    ...
    if (p.kind != TraceKind::WwisePost && p.kind != TraceKind::SourceRecord)
        return;
    Census049 c{};
    c.kind = p.kind == TraceKind::WwisePost ? 1u : 2u;
    c.event = p.eventId;
    c.playing = p.kind == TraceKind::WwisePost ? static_cast<std::uint32_t>(p.result)
                                               : static_cast<std::uint32_t>(p.nestedPostResult);
    c.key1 = p.eventId;
    c.key2 = p.objectKey;
    c.key3 = c.playing;
    if (c.kind == 2) {
        c.key2 = p.body;
        c.key3 = p.sourceAfter;
    }
    c.body = p.body;
    c.objectKey = p.objectKey;
    c.source = p.source;
    c.caller = p.caller;
    c.last = p.tick;
    ObserveCensus049(c);
...
bool CaptureNormalAudioTransform(std::int64_t body, std::uint32_t (&out)[8]) {
    ...
    if (link == 0 || !IsReadable(reinterpret_cast<const void*>(link + 0x18), sizeof(std::uintptr_t))) {
        return false;
    }

    std::uintptr_t transform = 0;
    std::memcpy(&transform, reinterpret_cast<const void*>(link + 0x18), sizeof(transform));
    if (transform == 0 || !IsReadable(reinterpret_cast<const void*>(transform + 0x110), sizeof(out))) {
        return false;
    }
...
void FlushTraceRecords() {
    ...
    bool expected = false;
    if (!g_flushBusy.compare_exchange_strong(expected, true, std::memory_order_acq_rel,
                                             std::memory_order_relaxed)) {
        return;
    }
    ...
    while (read < write && flushed < kMaxFlushPerBridge) {
        ...
        if (g_log && CanLog()) {
            ...
            case TraceKind::SourceRecord:
                g_log->info("SOURCE_RECORD,tick={},tid={},event=0x{:08X},body=0x{:016X},"
                            ...
                            "status=0x{:08X},valid={}",
                            p.tick, p.tid, p.eventId, static_cast<std::uint64_t>(p.body), p.sourceBefore,
                            p.sourceAfter, static_cast<std::uint64_t>(p.tableBase),
                            static_cast<std::uint64_t>(p.record), p.objectKey,
                            static_cast<std::uint64_t>(p.candidateRecord), p.candidateStoredSource,
                            p.candidateStoredKey64, p.derivedObjectKey, p.derivedKeyMatch, p.nestedObjectKey,
                            static_cast<std::uint32_t>(p.nestedPostResult), p.tlsMatch,
                            static_cast<std::uint32_t>(p.result), p.record != 0 ? 1 : 0);
                break;
            ...
            case TraceKind::SourceTransform: {
                ...
                g_log->info("SOURCE_XFORM,tick={},tid={},body=0x{:016X},source=0x{:08X},"
                            "x={:.6f},y={:.6f},z={:.6f}",
                            p.tick, p.tid, static_cast<std::uint64_t>(p.body), p.source, x, y, z);
                break;
            }
            ...
            case TraceKind::WwisePost:
                g_log->info(
                    ...
                    (p.foxEventId != 0 && p.foxEventId == p.eventId) ? 1 : 0, p.objectKey,
                    static_cast<std::uint32_t>(p.result), p.recordMatches, p.source,
                    static_cast<std::uint64_t>(p.body), static_cast<std::uint64_t>(p.record), p.tlsMatch,
                    static_cast<std::uint64_t>(p.caller));
                break;

            case TraceKind::GameObjectLookup:
                g_log->info("GAME_OBJECT_LOOKUP,tick={},tid={},object_key=0x{:016X},"
                            "object=0x{:016X},event=0x{:08X},source=0x{:08X},"
                            "body=0x{:016X},record=0x{:016X},post_result=0x{:08X},"
                            "caller=0x{:016X}",
                            p.tick, p.tid, p.objectKey, static_cast<std::uint64_t>(p.object), p.eventId,
                            p.source, static_cast<std::uint64_t>(p.body),
                            static_cast<std::uint64_t>(p.record), static_cast<std::uint32_t>(p.result),
                            static_cast<std::uint64_t>(p.caller));
                break;

            case TraceKind::PlayAction:
                g_log->info("PLAY_ACTION,tick={},tid={},action=0x{:016X},playing=0x{:08X},"
                            ...
                            p.tick, p.tid, static_cast<std::uint64_t>(p.action), p.playingId, p.matchMode,
                            static_cast<std::uint64_t>(p.object), p.objectKey, p.eventId, p.source,
                            static_cast<std::uint64_t>(p.body), static_cast<std::uint64_t>(p.record),
                            static_cast<std::uint64_t>(p.eventNode),
                            static_cast<std::uint64_t>(p.eventVtable),
                            static_cast<std::uint64_t>(p.executeV40), p.nodeType, p.actionRaw44,
                            static_cast<std::uint64_t>(p.caller));
                break;

            case TraceKind::PlayingEventPack:
                g_log->info("PLAYING_EVENT_PACK,tick={},tid={},playing=0x{:08X},event=0x{:08X},"
                            "out0=0x{:08X},out1=0x{:08X},tracked={},key=0x{:016X},"
                            "body=0x{:016X},source=0x{:08X},caller=0x{:016X}",
                            p.tick, p.tid, p.playingId, p.eventId, p.packOut0, p.packOut1, p.matchMode,
                            p.objectKey, static_cast<std::uint64_t>(p.body), p.source,
                            static_cast<std::uint64_t>(p.caller));
                break;
            ...
            case TraceKind::MatrixDirectBind:
                g_log->info(
                    ...
                    "caller=0x{:016X}",
                    p.tick, p.tid, p.route, p.playingId, p.eventId, p.objectKey, p.source,
                    static_cast<std::uint64_t>(p.body), static_cast<std::uint64_t>(p.record),
                    static_cast<std::uint64_t>(p.node), static_cast<std::uint64_t>(p.context),
                    static_cast<std::uint64_t>(p.contextObjectA8),
                    static_cast<std::uint64_t>(p.contextObject148), p.objectA8Match, p.object148Match,
                    p.dualIdentityMatch, p.sourceMask, p.destinationMask,
                    static_cast<std::uint64_t>(p.destination), p.hrtfRequested, p.hrtfApplied, p.hrtfFrames,
                    p.dryRequested, p.dryApplied, FloatFromBits(p.sourcePeakBits),
                    FloatFromBits(p.leftPeakBits), FloatFromBits(p.rightPeakBits),
                    FloatFromBits(p.azimuthBits), FloatFromBits(p.elevationBits),
                    FloatFromBits(p.distanceBits), static_cast<std::uint64_t>(p.caller));
                break;

            case TraceKind::ActiveRenderBind:
                g_log->info("ACTIVE_RENDER_BIND,tick={},tid={},playing=0x{:08X},event=0x{:08X},"
                            ...
                            static_cast<std::uint64_t>(p.node), static_cast<std::uint64_t>(p.context),
                            static_cast<std::uint64_t>(p.contextObjectA8),
                            static_cast<std::uint64_t>(p.contextObject148), p.renderMatchMask);
                break;

            case TraceKind::MatrixRouteCoverage:
                g_log->info("MATRIX_ROUTE,tick={},tid={},playing=0x{:08X},event=0x{:08X},"
                            ...
                            p.tick, p.tid, p.playingId, p.eventId, p.objectKey, p.source,
                            static_cast<std::uint64_t>(p.body), p.route, p.sourceMask, p.destinationMask,
                            p.fromActiveRender, static_cast<std::uint64_t>(p.node),
            ...
            case TraceKind::LowLevelRoute:
                g_log->info(
                    ...
                    p.tick, p.tid, p.playingId, p.eventId, p.objectKey, p.source,
                    static_cast<std::uint64_t>(p.body), p.lowViaMatrix, p.route, p.sourceMask,
                    p.destinationMask, p.lowFrames, FloatFromBits(p.gainStartBits),
                    FloatFromBits(p.gainDeltaBits), static_cast<std::uint64_t>(p.matrixCaller),
    ...
    if (g_log && now - prior >= kSummaryIntervalMs &&
        g_summaryTick.compare_exchange_strong(prior, now, std::memory_order_relaxed,
                                              std::memory_order_relaxed)) {
        g_log->info(
            ...
            now, g_sourceEvents.load(std::memory_order_relaxed),
            ...
            g_traceWrite.load(std::memory_order_relaxed), g_traceRead.load(std::memory_order_relaxed),
        ...
        g_log->info(
            ...
            now, g_holdWorldAuditionMute.load(std::memory_order_relaxed) ? 1 : 0,
...
int* __fastcall SourcePostEventHook(std::int64_t body, int* outResult, std::uint32_t eventId) {
    ...
    InspectRecordCandidate(sourceForRecord, tableBase, candidateRecord, candidateStoredSource,
                           candidateStoredKey);
    ...
    const std::uint64_t derivedKey =
        sourceForRecord != 0 ? static_cast<std::uint64_t>((sourceForRecord >> 8) + 1u) : 0ull;
    const bool derivedKeyMatch = hadPending && pendingCopy.sawMatchingWwisePost && derivedKey != 0 &&
                                 derivedKey == pendingCopy.matchingObjectKey;
    if (derivedKeyMatch) {
        g_sourceDerivedKeyMatches.fetch_add(1, std::memory_order_relaxed);
    }

    const std::uint32_t coveragePlaying = hadPending && pendingCopy.matchingPostResult > 0
                                              ? static_cast<std::uint32_t>(pendingCopy.matchingPostResult)
                                              : 0u;
    const std::uint64_t coverageKey =
        hadPending && pendingCopy.sawMatchingWwisePost ? pendingCopy.matchingObjectKey : objectKey;
    CoverageObserveSource(bodyPtr, eventId, sourceForRecord, coverageKey, coveragePlaying, GetTickCount64());
...
void __fastcall SourceTransformSyncHook(std::int64_t body) {
    ...
    std::uint32_t source = 0;
    if (bodyPtr != 0)
        ReadBodySource(bodyPtr, source);

    std::uint32_t payload[8]{};
    const auto now = GetTickCount64();

    // Update every transform; limit SOURCE_XFORM logging to once per 500 ms.
    const bool havePayload = bodyPtr != 0 && CaptureNormalAudioTransform(body, payload);
    if (havePayload && source != 0) {
        RememberFoxTransform(source, bodyPtr, payload, now);
    }

    const bool emitTransform = havePayload && ShouldEmitBodyTransform(bodyPtr, now);
    if (emitTransform) {
...
int __fastcall WwisePostGatewayHook(std::uint32_t eventId, std::uint64_t objectKey, std::uint32_t param3,
                                    std::uint64_t param4, std::uint64_t param5, int param6,
                                    std::uint64_t param7, std::uint32_t param8) {
    ...
    std::uint32_t source = 0;
    std::uintptr_t record = 0;
    const auto recordMatches = FindRecordsByObjectKey(objectKey, source, record);
    ...
    RememberKey(objectKey, eventId, source, body, record, recordMatches, now);

    const int result =
        g_wwisePostOriginal(eventId, objectKey, param3, param4, param5, param6, param7, param8);
...
void __fastcall PlayingEventPackHook(std::uint32_t* outPair, std::uint32_t playingId, std::uint32_t eventId) {
    ...
    IdentityMeta meta{};
    const bool tracked = LookupPlaying(playingId, meta, GetTickCount64()) && meta.eventId == eventId;
    if (tracked)
        g_playingEventPackTracked.fetch_add(1, std::memory_order_relaxed);
...
void __fastcall VoiceRenderNodeHook(std::int64_t audioBufferLike) {
    ...
    const bool playingMatch =
        playingId != 0 && LookupPlaying(playingId, playingMeta, now) && playingMeta.body != 0;
    const bool objectA8Match =
        objectA8 != 0 && LookupObject(objectA8, objectMetaA8, now) && objectMetaA8.body != 0;
    const bool object148Match =
        object148 != 0 && LookupObject(object148, objectMeta148, now) && objectMeta148.body != 0;
    ...
    if (selected.body != 0) {
        ...
        const auto selectedPlaying =
            playingId != 0 ? playingId
                           : (selected.postResult > 0 ? static_cast<std::uint32_t>(selected.postResult) : 0u);
        CoverageObserveRender(selected.body, selected.eventId, selected.source, selected.key, selectedPlaying,
                              matchMask, now);
...
std::int64_t __fastcall WwiseGameObjectLookupHook(std::int64_t manager, std::uint64_t objectKey) {
...
void __fastcall PlaybackActionDispatchHook(std::int64_t runtime, std::int64_t actionValue) {
    ...
    const auto eventNode = *reinterpret_cast<const std::uintptr_t*>(action + 0x08);
    const auto playingId = *reinterpret_cast<const std::uint32_t*>(action + 0x38);
    const auto actionRaw44 = *reinterpret_cast<const std::uint32_t*>(action + 0x44);
    const auto gameObject = *reinterpret_cast<const std::uintptr_t*>(action + 0x48);
    ...
    if (matchMode != 0) {
        ...
        if (eventNode != 0) {
            eventVtable = *reinterpret_cast<const std::uintptr_t*>(eventNode + 0x00);
            nodeType = static_cast<std::uint32_t>(*reinterpret_cast<const std::uint16_t*>(eventNode + 0x34));
            if (eventVtable != 0) {
                executeV40 = *reinterpret_cast<const std::uintptr_t*>(eventVtable + 0x40);
            }
        }
...
void __fastcall SinkPumpHook(std::int64_t sinkValue) {
    ...
    if (!SafeReadValue(g_exeBase + 0x2B9FA28ull, gate))
        g_gateUnreadable039.fetch_add(1);
    else if (gate == 0)
        g_gateZero039.fetch_add(1);
    else
        g_gateNonzero039.fetch_add(1);
...
unsigned PlayingStage048(std::uint32_t id, std::uint64_t now) {
    if (!id)
        return 0;
    auto* p = FindPlayingSlot(id, false);
    if (!p)
        return 1;
    const auto t = p->tick.load();
    if (!t || (now > t && now - t > kRecentIdentityTtlMs))
        return 2;
...
unsigned ObjectStage048(std::uintptr_t object, std::uint64_t now) {
    if (!object)
        return 0;
    auto* p = FindObjectSlot(object, false);
    if (!p)
        return 1;
    const auto t = p->tick.load();
    if (!t || (now > t && now - t > kRecentIdentityTtlMs))
...
unsigned Activity048(unsigned mode, std::uint64_t now, std::uint64_t applied, std::uint64_t attempted) {
    if (!mode)
        return 0;
    if (applied && now >= applied && now - applied <= 1500)
        return 1;
    if (attempted && now >= attempted && now - attempted <= 1500)
        return 2;
    return 3;
}

// global world-source trial. 0=native, 1=hybrid, 2=pure HRIR.
std::atomic<unsigned> g_worldMode047{
    2}; // default: pure (DF-EQ filters make the hybrid crossover unnecessary)
std::atomic<std::uint64_t> g_worldEpoch047{1}, g_worldAttempts047{0}, g_worldApplied047{0},
    g_worldFallback047{0};
std::atomic<std::uint64_t> g_worldMono047{0}, g_worldStereo047{0}, g_worldQuad047{0}, g_worldLayout047{0};

bool WorldEligible047(bool dual, std::uintptr_t body, std::uint32_t playing, unsigned route,
                      unsigned sourceMask, unsigned destinationMask, std::uint16_t frames) {
...
void PrepareWorldHistory047(BinauralHistoryState& history, std::uintptr_t body, std::uint32_t playing,
    ...
    if (history.ownerBody047 != body || history.ownerPlaying047 != playing ||
        ...
        history.ownerBody047 = body;
        history.ownerPlaying047 = playing;
        history.ownerEpoch047 = epoch;
    }
...
void Control047() {
    ...
    unsigned mode = g_worldMode047.load();
    const auto old = mode;
    if (f9 && !prior9)
        mode = 0;
    else if (f11 && !prior11) {
        preferred = preferred == 1 ? 2 : 1;
        mode = preferred;
    } else if (f8 && !prior8)
        mode = mode ? 0 : preferred;
    if (mode != old) {
        ++g_worldEpoch047;
        g_worldMode047.store(mode);
    }
    const auto now = GetTickCount64();
    if (g_log && (mode != old || now - last >= 60000)) {
        g_log->info(
            "WORLD049,mode={},attempts={},applied={},fallback={},mono={},stereo={},quad={},unsupported_layout={},history_resets={}",
            mode, g_worldAttempts047.load(), g_worldApplied047.load(), g_worldFallback047.load(),
            g_worldMono047.load(), g_worldStereo047.load(), g_worldQuad047.load(), g_worldLayout047.load(),
            g_binauralHistoryResets.load());
        g_log->info(
            "COVERAGE048,tick={},route2={},dual={},playing_missing_key={},playing_no_slot={},playing_expired={},playing_no_body={},playing_bound={},object_missing_key={},object_no_slot={},object_expired={},object_no_body={},object_bound={},activity={}",
            now, g_route2Seen048.load(), g_route2Dual048.load(), g_playingStages048[0].load(),
            g_playingStages048[1].load(), g_playingStages048[2].load(), g_playingStages048[3].load(),
            g_playingStages048[4].load(), g_objectStages048[0].load(), g_objectStages048[1].load(),
            g_objectStages048[2].load(), g_objectStages048[3].load(), g_objectStages048[4].load(),
...
std::atomic<std::uint64_t> g_nativeUpdates050{0}, g_nativeValid050{0}, g_nativeDrops050{0},
    g_nativeMulti050{0};
std::atomic<std::uint64_t> g_nativeCandidates050{0}, g_nativeApplied050{0}, g_nativeMuted050{0},
    g_nativeFallback050{0}, g_nativeAmbiguous050{0};
...
std::atomic<std::uint64_t> g_setOutOfScope052{0}, g_setKeyMismatch052{0}, g_setMode052{0}, g_setStorage052{0},
    g_setNonFinite052{0}, g_setCountZero052{0};
...
struct OosSample052 {
    std::uintptr_t caller = 0, object = 0;
    std::uint64_t key = 0;
    std::uint32_t mode = 0;
...
std::atomic<std::uint64_t> g_bedApplied055{0}, g_bedMuted055{0}, g_bedFail055{0}, g_bedBusy055{0},
    g_bedPeakGuard055{0};
...
void NoteOosCaller052(std::uintptr_t caller) {
    for (auto i = 0u; i < g_oosCaller052.size(); ++i) {
        ...
        if (cur == caller) {
            ++g_oosCallerHits052[i];
            return;
        }
        if (cur == 0) {
            std::uintptr_t expected = 0;
            if (g_oosCaller052[i].compare_exchange_strong(expected, caller) || expected == caller) {
                ++g_oosCallerHits052[i];
                return;
            }
        }
...
using NativeDispatch050 = std::uint64_t(__fastcall*)(std::uintptr_t, std::uint64_t, const float*,
...
std::size_t NativeIndex050(std::uintptr_t object) {
    return ((object >> 4) ^ (object >> 16)) & 4095;
}
...
void StoreNative050(std::uintptr_t object, std::uint64_t key, const float* xyz, bool reset) {
    if (g_nativeLock050.test_and_set(std::memory_order_acquire)) {
        ++g_nativeDrops050;
        ++g_nativeEpoch050;
        return; // invalidate old observations on a lost update
    }
    auto& slot = g_nativePositions050[NativeIndex050(object)];
    if (reset || slot.object != object || slot.key != key) {
        slot = NativePosition050{};
        slot.generation = ++g_nativeGeneration050;
    }
    slot.object = object;
    slot.key = key;
    slot.epoch = g_nativeEpoch050.load();
...
bool CopyNative050(std::uintptr_t object, NativePosition050& out) {
    if (!object || g_nativeLock050.test_and_set(std::memory_order_acquire))
        return false;
...
void __fastcall NativeSetHook050(std::uintptr_t object, const float* positions, std::uint16_t count,
                                 std::uint32_t mode) {
    // Capture from the consumer, not the producer queue, so position order matches playback.
    float xyz[3]{};
    const bool captured =
        count == 1 && positions && SafeReadValue(reinterpret_cast<std::uintptr_t>(positions), xyz);
    g_nativeSetOriginal050(object, positions, count, mode);
    ++g_nativeUpdates050;
    if (count > 1)
        ++g_nativeMulti050;
    std::uint64_t key = 0;
    std::uint16_t storedCount = 0;
    std::uintptr_t data = 0;
    float stored[3]{};
    bool valid = captured && mode <= 2 && g_nativeScope050 && SafeReadValue(object + 0x70, key) && key &&
                 key == g_nativeScopeKey050 && SafeReadValue(object + 0x10, storedCount) &&
                 storedCount == 1 && SafeReadValue(object + 8, data) && data && SafeReadValue(data, stored);
    ...
    if (!valid) {
        if (count == 0)
        ...
        } // already counted as multi_updates
            ...
            ++g_setMode052;
        else if (!key || key != g_nativeScopeKey050)
            ++g_setKeyMismatch052;
        else if (captured && !(std::isfinite(xyz[0]) && std::isfinite(xyz[1]) && std::isfinite(xyz[2])))
            ++g_setNonFinite052;
        else
            ++g_setStorage052;
    }
...
std::uint64_t __fastcall NativeDispatchHook050(std::uintptr_t manager, std::uint64_t key,
                                               const float* positions, std::uint16_t count,
                                               std::uint32_t mode) {
    const bool prior = g_nativeScope050;
    const auto priorKey = g_nativeScopeKey050;
    g_nativeScope050 = true;
    g_nativeScopeKey050 = key;
    const auto result = g_nativeDispatchOriginal050(manager, key, positions, count, mode);
    g_nativeScope050 = prior;
    g_nativeScopeKey050 = priorKey;
    return result;
}

bool ReadNative050(std::uintptr_t object, NativePosition050& out) {
    if (!CopyNative050(object, out))
        return false;
    std::uint64_t key = 0;
    std::uint16_t count = 0;
    return SafeReadValue(object + 0x70, key) && key == out.key && SafeReadValue(object + 0x10, count) &&
           count == 1;
}
...
bool ReadDirect053(std::uintptr_t object, NativePosition050& out) {
    ...
    std::uint64_t key = 0;
    std::uint16_t count = 0;
    std::uint8_t flags13 = 0;
    std::uintptr_t data = 0;
    float xyz[3]{};
    if (!SafeReadValue(object + 0x70, key) || !key || !SafeReadValue(object + 0x10, count) || count != 1 ||
        !SafeReadValue(object + 0x13, flags13) || (flags13 & 7u) > 2u || !SafeReadValue(object + 8, data) ||
        !data || !SafeReadValue(data, xyz))
        return false;
    for (unsigned i = 0; i < 3; ++i)
        if (!std::isfinite(xyz[i]) || std::fabs(xyz[i]) >= 1000000.0f)
            return false;
    if (xyz[0] == 0.0f && xyz[1] == 0.0f && xyz[2] == 0.0f)
        return false; // default pose is not evidence
    out = NativePosition050{};
    out.object = object;
    out.key = key;
    out.valid = true;
    out.epoch = g_nativeEpoch050.load();
    out.generation =
        (key * 0x9E3779B97F4A7C15ull) ^ static_cast<std::uint64_t>(object) ^ 0x8000000000000000ull;
    out.xyz[0] = xyz[0];
    out.xyz[1] = xyz[1];
    out.xyz[2] = -xyz[2]; // native Z is flipped
    return true;
}

bool ReadAny053(std::uintptr_t object, NativePosition050& out, bool& direct) {
    direct = false;
    if (ReadNative050(object, out))
        return true;
    if (ReadDirect053(object, out)) {
        direct = true;
        return true;
    }
    return false;
...
bool ChooseNative050(std::uintptr_t a, std::uintptr_t b, NativePosition050& out) {
    ...
    if (a && b && a != b) {
        if (ax && by) {
            ...
            ++g_pair148Fail052[ClassifyNative052(b)];
            ++g_pairA8Preferred053;
        } else if (by) {
            ++g_pairA8Fail052[ClassifyNative052(a)];
            ++g_nativeAmbiguous050;
            return false;
        } else {
            ++g_pairBothFail052;
...
void ControlNative050() {
    static bool prior10 = false;
    static std::uint64_t last = 0;
    const bool f10 = AudioKey062(VK_F10);
    bool enabled = g_nativeEnabled050.load();
    bool changed = false;
    if (f10 && !prior10) {
        const bool wide = g_nativeWide051.load();
        if (!enabled) {
            enabled = true;
            g_nativeWide051.store(false);
        } else if (!wide) {
            g_nativeWide051.store(true);
        } else {
            enabled = false;
            g_nativeWide051.store(false);
        }
        g_nativeEnabled050.store(enabled);
        ++g_worldEpoch047;
        changed = true;
    }
    ...
    if (f4 && !prior4) {
        g_elevation059.store(!g_elevation059.load());
        changed = true;
    }
    prior4 = f4;
    ...
    if (f5 && !prior5) {
        if (g_bedEnabled055.load() && !g_bedSkip71_057.load())
            g_bedSkip71_057.store(true); // ALL -> AMBIENCE
        else if (g_bedEnabled055.load()) {
            g_bedEnabled055.store(false);
            g_bedSkip71_057.store(false);
        } // AMBIENCE -> OFF
        else {
            g_bedEnabled055.store(true);
            g_bedSkip71_057.store(false);
        } // OFF -> ALL
        changed = true;
    }
    ...
    if (g_nativeMute050.exchange(mute) != mute) {
        ++g_worldEpoch047;
        changed = true;
    }
    prior10 = f10;
    ...
    if (g_log && (changed || now - last >= 10000)) {
        g_log->info(
            "NATIVE050,tick={},ready={},enabled={},mute={},updates={},valid_updates={},multi_updates={},drops={},candidates={},applied={},muted={},fallback={},ambiguous={}",
            now, g_nativeReady050.load() ? 1 : 0, enabled ? 1 : 0, mute ? 1 : 0, g_nativeUpdates050.load(),
            g_nativeValid050.load(), g_nativeMulti050.load(), g_nativeDrops050.load(),
            g_nativeCandidates050.load(), g_nativeApplied050.load(), g_nativeMuted050.load(),
            g_nativeFallback050.load(), g_nativeAmbiguous050.load());
        g_log->info(
            "NATIVE051,tick={},state={},cand_mono={},cand_stereo={},cand_quad={},applied_mono={},applied_stereo={},applied_quad={}",
        ...
        auto a5 = [](const std::array<std::atomic<std::uint64_t>, 5>& v) {
            return fmt::format("{}/{}/{}/{}", v[1].load(), v[2].load(), v[3].load(), v[4].load());
        };
        g_log->info(
            "DIAG052,tick={},set_oos={},set_keymis={},set_mode={},set_storage={},set_nan={},set_count0={},"
            ...
            now, g_setOutOfScope052.load(), g_setKeyMismatch052.load(), g_setMode052.load(),
            g_setStorage052.load(), g_setNonFinite052.load(), g_setCountZero052.load(), a5(g_singleFail052),
            a5(g_pairA8Fail052), a5(g_pair148Fail052), g_pairBothFail052.load(), g_pairKeyDiff052.load(),
            g_pairPosDiff052[0].load(), g_pairPosDiff052[1].load(), g_pairPosDiff052[2].load(),
            g_pairPosDiff052[3].load(), g_gateNoPlaying052.load(), g_gateDest052.load(),
            g_gateLayout052.load(), g_gateLfe052.load(), g_foxMuted052.load());
        g_log->info("NATIVE053,tick={},direct_reads={},a8_preferred_over_unpositioned_148={}", now,
                    g_nativeDirect053.load(), g_pairA8Preferred053.load());
        {
            ...
            g_log->info("PEAK066,tick={},enabled={},guarded_blocks={}", now, g_peakGuard066.load() ? 1 : 0,
                        g_peakGuarded066.load());
            g_log->info(
                "BED057,tick={},state={},level_match={},makeup_over_3db_blocks={},implicit_fold_blocks={}",
                now, !g_bedEnabled055.load() ? "off" : (g_bedSkip71_057.load() ? "ambience_only" : "all"),
                g_bedLevelMatch057.load() ? 1 : 0, g_bedMakeupLarge057.load(), g_bedImplicit058.load());
            g_log->info(
                "BED055,tick={},enabled={},applied={},quad={},five={},seven={},other={},muted={},fail={},order_mismatch={},peak_guard={}",
                now, g_bedEnabled055.load() ? 1 : 0, g_bedApplied055.load(), g_bedQuad055.load(),
                g_bedFive055.load(), g_bedSeven055.load(), g_bedOther055.load(), g_bedMuted055.load(),
                g_bedFail055.load(), g_bedOrderMismatch056.load(), g_bedPeakGuard055.load());
            g_log->info("ELEV059,tick={},enabled={},blocks_below_-20={},level={},above_+20={}", now,
                        g_elevation059.load() ? 1 : 0, g_elevBelow059.load(), g_elevLevel059.load(),
                        g_elevAbove059.load());
            g_log->info(
                "MIX056,tick={},mode={},filter_crossfades={},nonfinite_rejects={},stereo_wide_blocks={},history_resets={}",
                now, g_worldMode047.load() == 2 ? "pure" : (g_worldMode047.load() == 1 ? "hybrid" : "native"),
                g_filterCrossfades056.load(), g_limiterEngaged056.load(), g_stereoWide056.load(),
                g_binauralHistoryResets.load());
            if (faults != reportedFaults) {
                g_log->warn("READ_FAULT054,tick={},total={}", now, faults);
                reportedFaults = faults;
            }
        }
        ...
        if (now - lastCallers >= 60000 || changed) {
            lastCallers = now;
            for (auto i = 0u; i < g_oosCaller052.size(); ++i)
                if (auto c = g_oosCaller052[i].load())
                    g_log->info("CALLER052,slot={},caller=0x{:016X},hits={}", i, c,
                                g_oosCallerHits052[i].load());
            if (g_oosCallerOverflow052.load())
                g_log->info("CALLER052,overflow={}", g_oosCallerOverflow052.load());
            static std::uint32_t written = 0;
            const auto have =
                (std::min)(g_oosSamples052.load(), static_cast<std::uint32_t>(g_oosSampleRows052.size()));
            for (; written < have; ++written) {
                const auto& r = g_oosSampleRows052[written];
                g_log->info(
                    "OOS052,n={},caller=0x{:016X},object=0x{:X},key=0x{:X},mode={},count={},x={:.3f},y={:.3f},z={:.3f}",
...
std::atomic<std::uint64_t> g_hrtfEpoch043{0}, g_hrtfAttempts043{0}, g_hrtfApplied043{0}, g_hrtfFallback043{0},
    g_hrtfLayoutSkipped043{0};
...
bool SameCandidate042(const Candidate042& c, const IdentityMeta& m, std::uint32_t playing) {
    return c.meta.body == m.body && c.meta.eventId == m.eventId && c.meta.key == m.key &&
           c.meta.source == m.source && c.playing == playing;
}

bool KnownLayout042(std::uint32_t mask) {
    return mask == 3 || mask == 4 || mask == 0x33;
}

unsigned Audition042(const IdentityMeta& meta, std::uint32_t playing, bool dual, std::uint32_t route,
                     std::uint32_t sourceMask, std::uint32_t destinationMask, std::uint16_t frames,
                     std::uint64_t now) {
    if (!dual || !meta.body || !playing || !frames || (route != 1 && route != 2) ||
        !KnownLayout042(sourceMask) || !KnownLayout042(destinationMask))
        return false;
    if (g_candidateLock042.test_and_set(std::memory_order_acquire)) {
        ++g_candidateBusy042;
        return false;
    }
    std::size_t i = 0;
    for (; i < g_candidateCount042; ++i)
        if (SameCandidate042(g_candidates042[i], meta, playing))
            break;
    if (i == g_candidateCount042 && i < g_candidates042.size()) {
        g_candidates042[i].meta = meta;
        g_candidates042[i].playing = playing;
        ++g_candidateCount042;
    }
    unsigned mode = 0;
    if (i < g_candidates042.size()) {
        auto& c = g_candidates042[i];
        c.last = now;
        ++c.hits;
        if (route == 2) {
        ...
        if (static_cast<int>(i) == g_selected042) {
            if (g_muteHeld042.load(std::memory_order_relaxed))
                mode = 1;
            else if (g_bypassHeld044.load(std::memory_order_relaxed) ||
                     g_pureHeld046.load(std::memory_order_relaxed) ||
                     g_hrtfHeld043.load(std::memory_order_relaxed)) {
                if (route == 2 && sourceMask == 4 && destinationMask == 3)
                    mode = g_bypassHeld044.load() ? 3u : (g_pureHeld046.load() ? 4u : 2u);
                else
                    ++g_hrtfLayoutSkipped043;
            }
        }
        if (mode == 1) {
            ++c.muted;
...
void Control042() {
    static bool priorNext = false, priorClear = false, priorMute = false, priorHrtf = false,
                priorBypass = false, priorPure = false;
    ...
    const auto now = GetTickCount64();
    Candidate042 snapshot{};
    int index = -1;
    unsigned live = 0;
    bool changed = false;
    // Retry on the next listener update if the lock is busy.
    if (g_candidateLock042.test_and_set(std::memory_order_acquire)) {
        g_muteHeld042.store(false);
        g_hrtfHeld043.store(false);
        g_bypassHeld044.store(false);
        g_pureHeld046.store(false);
        return;
    }
    for (std::size_t i = 0; i < g_candidateCount042; ++i)
        if (Eligible045(g_candidates042[i], now))
            ++live;
    if (clear && !priorClear) {
        g_selected042 = -1;
        changed = true;
        ...
        for (std::size_t n = 1; n <= g_candidateCount042; ++n) {
            const auto i = static_cast<std::size_t>((start + static_cast<int>(n)) %
                                                    static_cast<int>(g_candidateCount042));
            const auto& c = g_candidates042[i];
            if (Eligible045(c, now)) {
                g_selected042 = static_cast<int>(i);
                break;
            }
        }
        changed = true;
    ...
    index = g_selected042;
    if (index >= 0)
        snapshot = g_candidates042[static_cast<std::size_t>(index)];
    g_candidateLock042.clear(std::memory_order_release);
    // Keep the selected target while muted.
    static bool requireRelease = false;
    if (changed && (held || hrtf || bypass || pure))
        requireRelease = true;
    if (!held && !hrtf && !bypass && !pure)
    ...
    if (hrtfArmed != g_hrtfHeld043.load())
        ++g_hrtfEpoch043;
    g_hrtfHeld043.store(hrtfArmed);
    const bool pureArmed = pure && !bypass && !held && !requireRelease && ready045;
    if (pureArmed != g_pureHeld046.load())
        ++g_hrtfEpoch043;
    g_pureHeld046.store(pureArmed);
    g_bypassHeld044.store(bypass && !held && !requireRelease && ready045);
    if (g_log && (changed || held != priorMute || hrtf != priorHrtf || bypass != priorBypass ||
                  pure != priorPure || now - lastSummary >= 2000)) {
        g_log->info(
            "AUDITION046,changed={},selected={},live={},f7={},armed={},body=0x{:X},event=0x{:X},playing={},source=0x{:X},key=0x{:X},hits={},muted={},route1_muted={},route2_muted={},age_ms={},lock_busy={}",
            changed ? 1 : 0, index, live, held ? 1 : 0, g_muteHeld042.load() ? 1 : 0, snapshot.meta.body,
            snapshot.meta.eventId, snapshot.playing, snapshot.meta.source, snapshot.meta.key, snapshot.hits,
            snapshot.muted, snapshot.route1, snapshot.route2, index >= 0 ? now - snapshot.last : 0,
            g_candidateBusy042.load());
        g_log->info("SELECTION046,eligible={},mono_hits={},mono_age_ms={},available_mono={}",
                    ready045 ? 1 : 0, snapshot.monoHits,
                    index >= 0 && now >= snapshot.monoLast ? now - snapshot.monoLast : 0, live);
        g_log->info(
            "HRTF046,f8={},armed={},attempts={},applied={},fallback={},layout_skipped={},azimuth_deg={},distance={}",
            hrtf ? 1 : 0, hrtfArmed ? 1 : 0, g_hrtfAttempts043.load(), g_hrtfApplied043.load(),
            g_hrtfFallback043.load(), g_hrtfLayoutSkipped043.load(), FloatFromBits(g_azimuth043.load()),
            FloatFromBits(g_distance043.load()));
        g_log->info(
            "BYPASS046,f10={},armed={},attempts={},applied={},fallback={},metrics_mode={},input_peak={},left_peak={},right_peak={},input_rms={},left_rms={},right_rms={}",
            bypass ? 1 : 0, g_bypassHeld044.load() ? 1 : 0, g_bypassAttempts044.load(),
            g_bypassApplied044.load(), g_bypassFallback044.load(), g_metricsMode044.load(),
            FloatFromBits(g_inputPeak044.load()), FloatFromBits(g_leftPeak044.load()),
            FloatFromBits(g_rightPeak044.load()), FloatFromBits(g_inputRms044.load()),
            FloatFromBits(g_leftRms044.load()), FloatFromBits(g_rightRms044.load()));
        g_log->info("PURE046,f11={},armed={},attempts={},applied={},fallback={}", pure ? 1 : 0,
                    pureArmed ? 1 : 0, g_pureAttempts046.load(), g_pureApplied046.load(),
                    g_pureFallback046.load());
        g_log->flush();
        lastSummary = now;
    }
    priorNext = next;
...
constexpr float kBedNormalise055 =
    0.70710678f; // binaural pair carries ~2x the energy of a one-sided downmix row

struct BedState055 {
    std::uintptr_t node = 0, destination = 0;
    std::uint32_t mask = 0;
...
BedState055* FindBedState055(std::uintptr_t node, std::uintptr_t destination, std::uint32_t mask,
                             std::uint64_t) {
    const std::uint64_t now = ++g_bedClock055;
    BedState055* oldest = &g_bedStates055[0];
    for (auto& st : g_bedStates055) {
        if (st.node == node && st.destination == destination && st.mask == mask) {
            st.lastTick = now;
            return &st;
        }
        if (st.lastTick < oldest->lastTick)
            oldest = &st;
    }
    *oldest = BedState055{};
    oldest->node = node;
...
bool ApplyBedVirtualizer055(std::int64_t destinationValue, std::int64_t sourceBufferValue,
                            const float* coefficientBlock, std::uintptr_t node, std::uintptr_t destination,
                            std::uint32_t sourceMask, std::uint64_t now, std::uint32_t eventId057 = 0,
                            std::uint32_t playingId057 = 0) {
    const auto sourceBuffer = static_cast<std::uintptr_t>(sourceBufferValue);
    std::uintptr_t data = 0;
    std::uint16_t maxFrames16 = 0, validFrames16 = 0;
    if (!sourceBuffer || !coefficientBlock || !SafeReadValue(sourceBuffer + 0x00, data) ||
        !SafeReadValue(sourceBuffer + 0x10, maxFrames16) ||
        !SafeReadValue(sourceBuffer + 0x12, validFrames16) || !data)
        return false;
    ...
    if (!frames || frames > maxFrames || maxFrames > kBinauralMaxFrames)
        return false;
    if (!IsReadable(coefficientBlock, channels * 16u * sizeof(float)) ||
        !IsReadable(reinterpret_cast<const void*>(data),
                    static_cast<std::size_t>(channels) * maxFrames * sizeof(float)))
        return false;
    for (unsigned i = 0; i < channels * 16u; ++i)
        if (!std::isfinite(coefficientBlock[i]))
            return false;

    // Buffer channel index -> speaker bit (increasing bit order, LFE moved last).
    unsigned bits[kBedMaxChannels055]{};
    unsigned n = 0;
    for (unsigned b = 0; b < 11; ++b)
        if (b != 3 && (sourceMask & (1u << b)))
            bits[n++] = b;
    if (sourceMask & 0x8u)
        bits[n++] = 3;
    if (n != channels)
    ...
    for (unsigned ch = 0; ch < channels; ++ch) {
        ...
        const bool leftBit = bit == 0 || bit == 4 || bit == 6 || bit == 9,
                   rightBit = bit == 1 || bit == 5 || bit == 7 || bit == 10;
        if ((leftBit && r > l * 1.05f + 1.0e-4f) || (rightBit && l > r * 1.05f + 1.0e-4f) ||
            (bit == 2 && std::fabs(l - r) > 0.25f * (std::max)(l, r))) {
            ++g_bedOrderMismatch056;
            return false;
        }
    }
    ...
    for (unsigned ch = 0; ch < channels; ++ch) {
        ...
        const unsigned bit = bits[ch];
        const bool zeroRow =
            std::fabs(row[0]) + std::fabs(row[1]) + std::fabs(row[8]) + std::fabs(row[9]) < 1.0e-6f;
        if (!zeroRow || bit == 0 || bit == 1 || bit == 3 ||
            !g_bedImplicitFold058.load(std::memory_order_relaxed))
            continue;
        ...
        case 8:
            kl = kr = 0.70710678f;
            break; // centre / back centre
        case 4:
        case 9:
        case 6:
            kl = 0.70710678f;
            break; // left surrounds / FLC
        case 5:
        case 10:
        case 7:
            kr = 0.70710678f;
            break; // right surrounds / FRC
        default:
            break;
        }
        eff[ch][0] = vol0 * kl;
    ...
    for (unsigned ch = 0; ch < channels; ++ch)
        anyImplicit |= implicitRow[ch];
    if (anyImplicit)
        ++g_bedImplicit058;

    BedState055* state = FindBedState055(node, destination, sourceMask, now);

    float* left = g_directLeftEar;
    float* right = g_directRightEar;
    ...
    float* origLeft = g_bedOrigLeft057;
    float* origRight = g_bedOrigRight057;
    std::memset(origLeft, 0, frames * sizeof(float));
    std::memset(origRight, 0, frames * sizeof(float));
    ...
    for (unsigned ch = 0; ch < channels && ok; ++ch) {
        ...
        if (!ok)
            break;
        alignas(16) float bedLeft061[HRIRSET::kTapCount];
        alignas(16) float bedRight061[HRIRSET::kTapCount];
        BuildHrirWithEmphasis064(static_cast<float>(angle), 0.0f, 0.0f, bedLeft061,
                                 bedRight061); // beds = plain filters
    ...
    for (std::uint32_t i = 0; i < frames; ++i) {
        origEnergy +=
            static_cast<double>(origLeft[i]) * origLeft[i] + static_cast<double>(origRight[i]) * origRight[i];
    ...
    if (g_bedLevelMatch057.load(std::memory_order_relaxed) && origEnergy > 1.0e-6 * frames &&
        outEnergy > 1.0e-12) {
        // Limit calibration to 0.05 dB per block and +/-3 dB.
        const float want =
            (std::min)(1.41f, (std::max)(0.71f, static_cast<float>(std::sqrt(origEnergy / outEnergy))));
    ...
    if (makeup0 != 1.0f || makeup1 != 1.0f)
        for (std::uint32_t i = 0; i < frames; ++i) {
            const float g = makeup0 + (makeup1 - makeup0) * ((static_cast<float>(i) + 1.0f) * invFrames);
            left[i] *= g;
            right[i] *= g;
        }
...

    {
        ...
        if (g_log && g_debugLog063.load(std::memory_order_relaxed) && now - prev >= 2000 &&
            last.compare_exchange_strong(prev, now)) {
            auto db = [](double e, std::uint32_t n) {
                return e > 1e-20 ? 10.0 * std::log10(e / n) : -200.0;
            };
            std::string chans;
            for (unsigned ch = 0; ch < channels; ++ch) {
                chans += fmt::format("{}:b{}:{:.1f}dB:l{:.3f}:r{:.3f}{};", ch, bits[ch],
                                     db(channelEnergy[ch], frames), eff[ch][0], eff[ch][1],
                                     implicitRow[ch] ? ":fold" : "");
            }
            double outAfter = 0.0;
            for (std::uint32_t i = 0; i < frames; ++i)
                outAfter += static_cast<double>(left[i]) * left[i] + static_cast<double>(right[i]) * right[i];
            g_log->info(
                "BEDDIAG057,tick={},node=0x{:X},mask=0x{:X},event=0x{:08X},playing={},frames={},orig_db={:.1f},binaural_db={:.1f},after_makeup_db={:.1f},makeup={:.2f},ch={}",
                now, node, sourceMask, eventId057, playingId057, frames, db(origEnergy, frames),
                db(outEnergy, frames), db(outAfter, frames), makeup1, chans);
        }
    }
    ...
    float target = 1.0f;
    if (peak > kHybridPeakGuard) {
        target = kHybridPeakGuard / peak;
        ++g_bedPeakGuard055;
    }
    if (target > state->limiterGain)
        target = (std::min)(target, state->limiterGain + 0.02f);
    ...
    if (!IsReadable(reinterpret_cast<const void*>(sourceBuffer), fakeBuffer.size()))
        return false;
    if (!GuardedCopy054(fakeBuffer.data(), reinterpret_cast<const void*>(sourceBuffer), fakeBuffer.size()))
        return false;
    *reinterpret_cast<std::uint32_t*>(fakeBuffer.data() + 0x08) = 0x00000004u;
...
void __fastcall SourceToDestinationMixHook(std::int64_t destinationValue, std::int64_t sourceBufferValue,
                                           const float* coefficientBlock) {
    const auto caller = reinterpret_cast<std::uintptr_t>(_ReturnAddress());
    ...
    if (caller == g_exeBase + 0x0035271Dull)
        route = 1;
    else if (caller == g_exeBase + 0x00352820ull)
        route = 2;
    else if (fromActiveRender)
        route = 3;
    ...
    if (tracked) {
        ...
        if (route == 1)
            g_matrixTrackedRoute1.fetch_add(1, std::memory_order_relaxed);
        else if (route == 2)
            g_matrixTrackedRoute2.fetch_add(1, std::memory_order_relaxed);
        else if (route == 3)
            g_matrixTrackedRoute3.fetch_add(1, std::memory_order_relaxed);
        else
            g_matrixTrackedOther.fetch_add(1, std::memory_order_relaxed);

        if (playingMatch)
            g_matrixPlayingMatches.fetch_add(1, std::memory_order_relaxed);
        ...
        const bool finalTrackedWorldRoute = dualMatch && route == 2 && destinationMask == 0x00000003u;

        // This diagnostic branch does not apply HRTF.
        hrtfRequested = false;
        hrtfApplied = false;

        CoverageObserveMatrix(selected.body, selected.eventId, selected.source, selected.key, playingId,
                              route, sourceMask, destinationMask, caller, now);
        ...
        if ((hrtfRequested || muteRequested) && ShouldEmitMatrixBind(context, now)) {
            ...
            trace.contextObject148 = object148;
            trace.objectA8Match =
                (playingMatch && objectA8Match && SameIdentity(playingMeta, objectMetaA8)) ? 1u : 0u;
            trace.object148Match =
                (playingMatch && object148Match && SameIdentity(playingMeta, objectMeta148)) ? 1u : 0u;
    ...
    if (mode047 && g_nativeMute050.load() &&
        WorldEligible047(independent047, selected.body, playingId, route, sourceMask, destinationMask,
                         frames047)) {
        ...
        foxMuted052 = true;
        ++g_foxMuted052;
    } else if (mode047 && WorldEligible047(independent047, selected.body, playingId, route, sourceMask,
                                           destinationMask, frames047)) {
        ++g_worldAttempts047;
        g_lastAttempt048.store(now);
        ...
        if (frames047 == stride047 && stride047 <= kBinauralMaxFrames) {
            ...
            processed047 =
                ApplyWideWorldHrtf(destinationValue, sourceBufferValue, coefficientBlock, node, destination,
                                   sourceMask, selected, sourcePeak, leftPeak, rightPeak, azimuthDeg,
                                   elevationDeg, distance, hrtfFrames, false, mode047 == 2);
        }
        if (processed047) {
            ++g_worldApplied047;
            g_lastApplied048.store(now);
            if (sourceMask == 4)
                ++g_worldMono047;
            else if (sourceMask == 3)
                ++g_worldStereo047;
            else
                ++g_worldQuad047;
        } else
            ++g_worldFallback047;
               ...
               (destinationMask != 3 || (sourceMask != 4 && sourceMask != 3 && sourceMask != 0x33)))
        ++g_worldLayout047;
    ...
    if (!independent047 && route == 2 && frames047) {
        if (!playingId)
            ++g_gateNoPlaying052;
        else if (destinationMask != 3)
            ++g_gateDest052;
        else if (sourceMask == 8)
            ++g_gateLfe052;
        else if (!nativeLayout051)
            ++g_gateLayout052;
    }
    const bool nativeCandidate050 = !independent047 && route == 2 && playingId && nativeLayout051 &&
                                    destinationMask == 3 && frames047 &&
    ...
    if (!processed047 && nativeCandidate050 && nativeLayoutOn051 && mode047 && g_nativeEnabled050.load()) {
        ...
        if (frames047 == stride047 && stride047 <= kBinauralMaxFrames) {
            if (g_nativeMute050.load()) {
                ...
                if (history.ownerNativeGeneration050 != native050.generation)
                    history.initialized = false;
                ...
                processed047 = ApplyWideWorldHrtf(destinationValue, sourceBufferValue, coefficientBlock, node,
                                                  destination, sourceMask, selected, sourcePeak, leftPeak,
                                                  rightPeak, azimuthDeg, elevationDeg, distance, hrtfFrames,
                                                  false, mode047 == 2, native050.xyz);
                if (processed047) {
                    ++g_nativeApplied050;
                    ++g_worldApplied047;
                    g_lastApplied048.store(now);
                    if (sourceMask == 4) {
                        ++g_worldMono047;
                        ++g_nativeAppliedMono051;
                    } else if (sourceMask == 3) {
                        ++g_worldStereo047;
                        ++g_nativeAppliedStereo051;
                    } else {
                        ++g_worldQuad047;
                        ++g_nativeAppliedQuad051;
                    }
                }
            }
        }
    ...
    if (!processed047 && mode047 && g_bedEnabled055.load() && route == 2 && frames047 &&
        ...
        if (g_nativeMute050.load()) {
            ...
            processed047 = ApplyBedVirtualizer055(destinationValue, sourceBufferValue, coefficientBlock, node,
                                                  destination, sourceMask, now, selected.eventId, playingId);
            if (!processed047)
                ++g_bedFail055;
        }
    }
    if (!processed047)
        g_sourceToDestinationMixOriginal(destinationValue, sourceBufferValue, coefficientBlock);
    Census049 census049{};
    census049.kind = 3;
    census049.key1 = playingId;
    census049.key2 = context;
    census049.key3 = node;
    census049.last = now;
    census049.playing = playingId;
    census049.body = selected.body;
    census049.source = selected.source;
    census049.objectKey = selected.key;
    census049.event = selected.eventId;
    census049.node = node;
    census049.object = objectA8 ? objectA8 : object148;
    census049.caller = caller;
    census049.routes = 1u << route;
    census049.matches = matchMask;
    census049.srcMasks = sourceMask;
    census049.dstMasks = destinationMask;
    ...
    if (!census049.event) {
        IdentityMeta hint{};
        if (LookupPlaying(playingId, hint, now))
            census049.event = hint.eventId;
    }
    ObserveCensus049(census049);
    if (nativeCandidate050) {
        census049.kind = 4;
        census049.objectKey = native050.key;
        census049.object = native050.object;
        census049.body = 0;
        census049.source = 0;
        for (unsigned i = 0; i < 3; ++i)
            census049.xyzBits050[i] = FloatBits(native050.xyz[i]);
        ObserveCensus049(census049);
    }
    trace040.Emit(1);
...
void __fastcall AlternateMixCoreHook(std::int64_t frameInfoValue, std::int64_t sourceBufferValue,
                                     std::int64_t destinationBufferValue, char directSameMask,
                                     const float* coefficientBlock) {
    ...
    std::uintptr_t objectA8 = 0;
    std::uintptr_t object148 = 0;
    if (sourceBuffer != 0 && SafeReadValue(sourceBuffer + 0x40, node) && node != 0 &&
        SafeReadValue(node + 0x10, wrapper) && wrapper != 0 && SafeReadValue(wrapper + 0x18, context) &&
        context != 0) {
        SafeReadValue(context + 0x70, playingId);
        SafeReadValue(context + 0xA8, objectA8);
        SafeReadValue(context + 0x148, object148);
    ...
    Scope040 trace040(3, reinterpret_cast<std::uintptr_t>(_ReturnAddress()),
                      static_cast<std::uintptr_t>(frameInfoValue), sourceBuffer, destinationBuffer);
    trace040.Emit(0);
    g_alternateMixCoreOriginal(frameInfoValue, sourceBufferValue, destinationBufferValue, directSameMask,
                               coefficientBlock);
    trace040.Emit(1);
}

void __fastcall LowLevelChannelMixHook(std::int64_t sourcePcm, std::int64_t destinationPcm, float gainStart,
                                       float gainDelta, std::uint32_t frames) {
    const auto caller = reinterpret_cast<std::uintptr_t>(_ReturnAddress());
    ...
    if (tracked) {
        ...
        if (viaMatrix)
            g_lowLevelTrackedViaMatrix.fetch_add(1, std::memory_order_relaxed);
        else
            g_lowLevelTrackedViaRender.fetch_add(1, std::memory_order_relaxed);

        CoverageObserveLowLevel(selected.body, selected.eventId, selected.source, selected.key, playingId,
                                caller, viaMatrix, now);
    ...
    g_lowLevelChannelMixOriginal(sourcePcm, destinationPcm, gainStart, gainDelta, frames);
}
...
void __fastcall ListenerBridgeHook(std::int64_t owner) {
    ...
    g_holdWorldAuditionMute.store(false, std::memory_order_release);
    g_holdDirectHrtf.store(false, std::memory_order_release);
...
void DumpCoverageCensus() {
    ...
    for (const auto& slot : g_coverage) {
        if (slot.signature.load(std::memory_order_acquire) == 0)
            continue;
        ...
        if (lowCalls != 0)
            ++reachedLow;
        if (renderCalls != 0 && matrixCalls == 0)
            ++renderNoMatrix;
        if (matrixCalls != 0 && lowCalls == 0)
            ++matrixNoLow;

        g_log->info("ROUTE_CENSUS,body=0x{:016X},event=0x{:08X},source=0x{:08X},key=0x{:016X},"
                    ...
                    static_cast<std::uint64_t>(body), slot.eventId.load(std::memory_order_relaxed),
                    slot.source.load(std::memory_order_relaxed), slot.key.load(std::memory_order_relaxed),
                    ...
                    slot.sourcePosts.load(std::memory_order_relaxed), renderCalls,
                    slot.renderMatchMask.load(std::memory_order_relaxed), matrixCalls,
                    ...
                    lowCalls, slot.lowViaMatrix.load(std::memory_order_relaxed),
    ...
    g_log->info("ROUTE_CENSUS_SUMMARY,emitters={},reached_render={},reached_matrix={},reached_low={},"
                "no_render={},render_no_matrix={},matrix_no_low={}",
                emitters, reachedRender, reachedMatrix, reachedLow, noRender, renderNoMatrix, matrixNoLow);

    for (const auto& slot : g_matrixCallerCensus) {
        const auto caller = slot.caller.load(std::memory_order_acquire);
        if (caller == 0)
            continue;
        g_log->info("MATRIX_CALLER_CENSUS,caller=0x{:016X},calls={},tracked={}",
                    static_cast<std::uint64_t>(caller), slot.calls.load(std::memory_order_relaxed),
    ...
    for (const auto& slot : g_lowCallerCensus) {
        ...
        if (caller == 0)
            continue;
        g_log->info("LOWLEVEL_CALLER_CENSUS,caller=0x{:016X},calls={},tracked={}",
                    static_cast<std::uint64_t>(caller), slot.calls.load(std::memory_order_relaxed),
                    slot.tracked.load(std::memory_order_relaxed));
    }
}
...
bool GetAudioStatus(std::string& text) {
    if (!g_installed.load() || !g_overlay062.load())
        return false;
    ...
    const auto activity =
        Activity048(mode, GetTickCount64(), g_lastApplied048.load(), g_lastAttempt048.load());
    const char* state = activity == 0   ? "Original game audio"
                        : activity == 1 ? "Processing matched world sounds"
                        : activity == 2 ? "Matched sounds falling back to native"
                                        : "No eligible matched sounds recently";
    char buf[1024]{};
    std::snprintf(
        buf, sizeof(buf),
...
void Configure(bool overlay, bool hotkeys, float emphasis, bool debugLog, bool bedCentreDirect,
    ...
    { // arrival tap of the plain front filter (peak of the left+right energy)
        ...
        std::uint32_t best = 0;
        float peak = 0.0f;
        for (std::uint32_t t = 0; t < HRIRSET::kTapCount; ++t) {
            const float v = l[t] * l[t] + r[t] * r[t];
            if (v > peak) {
                peak = v;
                best = t;
            }
...
bool Install() {
    ...
    try {
        ...
        if (!g_log) {
            g_log = spdlog::basic_logger_mt("choomaudio_audio", g_pluginLogPath, true);
        }
        g_log->set_pattern("%v");
        ...
        g_log->info(
            "CONFIG069,audioOverlay={},audioHotkeys={},audioSpatialEmphasis={:.2f},audioDebugLog={},audioBedCentreDirect={},audioPeakGuard={},audioFoxHooks={},native={},listener={},mixer={},centre_delay_taps={}",
            g_overlay062.load() ? 1 : 0, g_hotkeys062.load() ? 1 : 0, g_emphasis062.load(),
            g_debugLog063.load() ? 1 : 0, g_bedCentreDirect064.load() ? 1 : 0, g_peakGuard066.load() ? 1 : 0,
            g_foxHooks068.load() ? 1 : 0, g_nativeHooks069.load() ? 1 : 0, g_listenerHook069.load() ? 1 : 0,
            g_mixerHook069.load() ? 1 : 0, g_bedCentreDelay064);
        {
            double hrirSum = 0.0;
            for (std::size_t d = 0; d < KEMAR360::kDirectionCount; ++d)
                for (std::size_t e = 0; e < 2; ++e)
                    for (std::size_t t = 0; t < KEMAR360::kTapCount; ++t)
                        hrirSum += std::fabs(KEMAR360::kHrir[d][e][t]);
            double setSum = 0.0;
            for (std::size_t d = 0; d < HRIRSET::kDirectionCount; ++d)
                for (std::size_t e = 0; e < 2; ++e)
                    for (std::size_t t = 0; t < HRIRSET::kTapCount; ++t)
                        setSum += std::fabs(HRIRSET::kHrir[d][e][t]);
            (void)hrirSum;
            g_log->info("HRIR061,set={},directions={},rings={},abs_sum={:.4f}", HRIRSET::kName,
                        HRIRSET::kDirectionCount, HRIRSET::kRingCount, setSum);
        }
        g_log->info(
            "MODE,default=pure_world_hrtf,f8=toggle_world_hrtf,f9=native_audio,f11=switch_hybrid_pure,f6=toggle_status_overlay,f7=hold_mute_all_spatialised_and_marker,f10=cycle_native_stereo_default_on,f5=cycle_beds_all_ambience_off,f4=toggle_elevation_default_on");
        g_log->info(
            "MODE,submit_target=runtime_sink_voice_vtable_plus_0xA8,selection=validated_sink_ring_buffers,global_mix_diagnostic=0,per_emitter_hrtf=verified_world_sources");
    } catch (...) {
        g_log = spdlog::default_logger();
        if (g_log) {
            g_log->error("AudioIdentityBridge: could not create dedicated log; using default logger");
        }
    }
    ...
    const bool nativeCtor =
        g_nativeHooks069.load() &&
        InstallOne(0x388590, reinterpret_cast<void*>(&NativeCtorHook050),
                   reinterpret_cast<void**>(&g_nativeCtorOriginal050), "NativeObject.Constructor050");
    const bool nativeSet =
        g_nativeHooks069.load() &&
        InstallOne(0x388e50, reinterpret_cast<void*>(&NativeSetHook050),
                   reinterpret_cast<void**>(&g_nativeSetOriginal050), "NativeObject.SetConsumedPosition050");
    const bool nativeDispatch =
        g_nativeHooks069.load() && InstallOne(0x34d220, reinterpret_cast<void*>(&NativeDispatchHook050),
                                              reinterpret_cast<void**>(&g_nativeDispatchOriginal050),
                                              "NativeObject.DispatchPosition050");
    g_nativeReady050.store(nativeCtor && nativeSet && nativeDispatch);
    if (g_log)
        g_log->info("NATIVE_INSTALL050,ctor={},setter={},dispatch={},ready={}", nativeCtor ? 1 : 0,
                    nativeSet ? 1 : 0, nativeDispatch ? 1 : 0, g_nativeReady050.load() ? 1 : 0);
    const bool sourceEvent =
        g_foxHooks068.load() &&
        InstallOne(kRvaSourcePostEvent, reinterpret_cast<void*>(&SourcePostEventHook),
                   reinterpret_cast<void**>(&g_sourcePostOriginal), "SoundSourceBody.PostEventInternal");

    const bool sourceTransform =
        g_foxHooks068.load() &&
        InstallOne(kRvaSourceTransformSync, reinterpret_cast<void*>(&SourceTransformSyncHook),
                   reinterpret_cast<void**>(&g_transformOriginal), "SoundSourceBody.TransformSync");

    const bool listener =
        g_listenerHook069.load() &&
        InstallOne(kRvaListenerBridge, reinterpret_cast<void*>(&ListenerBridgeHook),
                   reinterpret_cast<void**>(&g_listenerOriginal), "Camera.ActiveToSoundCoreListenerBridge");

    const bool wwisePost =
        g_foxHooks068.load() &&
        InstallOne(kRvaWwisePostGateway, reinterpret_cast<void*>(&WwisePostGatewayHook),
                   reinterpret_cast<void**>(&g_wwisePostOriginal), "Wwise.PostGateway.FUN_14033C050");

    const bool gameObjectLookup =
        g_foxHooks068.load() &&
        InstallOne(kRvaWwiseGameObjectLookup, reinterpret_cast<void*>(&WwiseGameObjectLookupHook),
    ...
    const bool playingEventPack =
        g_foxHooks068.load() &&
        InstallOne(kRvaPlayingEventPack, reinterpret_cast<void*>(&PlayingEventPackHook),
                   reinterpret_cast<void**>(&g_playingEventPackOriginal),
                   "Wwise.PlayingEventPack.FUN_140342060");

    const bool voiceRender =
        g_foxHooks068.load() &&
        InstallOne(kRvaVoiceRenderNode, reinterpret_cast<void*>(&VoiceRenderNodeHook),
                   reinterpret_cast<void**>(&g_voiceRenderOriginal), "Wwise.ActiveRenderNode.FUN_140352330");

    // Leave sink, alternate-mix and runtime submission diagnostics disabled.
    const bool sinkPump = false;

    const bool matrixMix =
        g_mixerHook069.load() &&
        InstallOne(kRvaSourceToDestinationMix, reinterpret_cast<void*>(&SourceToDestinationMixHook),
    ...
    if (g_log) {
        g_log->info(
            ...
            sourceEvent ? 1 : 0, sourceTransform ? 1 : 0, listener ? 1 : 0, wwisePost ? 1 : 0,
            gameObjectLookup ? 1 : 0, actionDispatch ? 1 : 0, playingEventPack ? 1 : 0, voiceRender ? 1 : 0,
            sinkPump ? 1 : 0, matrixMix ? 1 : 0, alternateMix ? 1 : 0, lowLevelMix ? 1 : 0);
        g_log->info("V047_GOAL,all_verified_supported_world_voices=1,unmatched_native=1");
        g_log->info(
        ...
        if (!g_debugLog063.load()) {
            ...
            g_log->info(
                "LOG063,diagnostics=off,set audioDebugLog=true in plugins/choomaudio.lua for full logging");
            g_log->set_level(spdlog::level::warn);
        }
        g_log->flush();
    }

    return sourceEvent || sourceTransform || listener || wwisePost || gameObjectLookup || actionDispatch ||
...
void Shutdown() {
    ...
    if (g_log) {
        ...
        g_log->info(
            ...
            g_sourceRecordsValid.load(std::memory_order_relaxed),
            g_sourceRecordsInvalid.load(std::memory_order_relaxed),
            g_sourceTransforms.load(std::memory_order_relaxed), g_wwisePosts.load(std::memory_order_relaxed),
