...
namespace ChoomAudioGuards {
	namespace NullGuard {
		...
		static void __fastcall Hook(void* self, void** obj, void* arg, void* hash) {
			if (obj == nullptr) {
				...
				if (n <= 20) {
					spdlog::warn("NULLGUARD FUN_140a0e300 skipped null object: caller=0x{:X} self=0x{:X} hash=0x{:X} count={}",
						reinterpret_cast<std::uintptr_t>(_ReturnAddress()), reinterpret_cast<std::uintptr_t>(self),
						reinterpret_cast<std::uintptr_t>(hash), n);
				}
				return;
			}
	...
	namespace ViewGuard {
		...
		static std::uintptr_t ReadContext(std::uintptr_t obj) {
			__try { return obj ? *reinterpret_cast<const std::uintptr_t*>(obj + 0x10) : 0; }
		...
		static void Install() {
			...
			std::int32_t rel = 0; std::memcpy(&rel, call + 1, sizeof(rel));
			...
			if (MH_CreateHook(fn, reinterpret_cast<void*>(&Hook), reinterpret_cast<void**>(&original)) != MH_OK ||
...
void Install(bool nullGuard,bool viewGuard){if(nullGuard)NullGuard::Install();if(viewGuard)ViewGuard::Install();}
}
