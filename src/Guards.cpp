...
static void Install() {
    ...
    if (nt->FileHeader.TimeDateStamp != 0x6A4CB898u) {
        spdlog::info("NULLGUARD not installed: exe timestamp 0x{:08X} is not 1.0.15.4 (0x6A4CB898)",
...
static void Install() {
    ...
    if (call[0] != 0xE8 || (target != want1 && target != want2)) {
        spdlog::warn(
            "VIEWGUARD not installed: call site at +0x1E0201C does not match (byte={:02X} target=0x{:X})",
            call[0], target);
        return;
    }
